...
uint32_t find_and_replace_all(std::string& text, const std::string& search,
...
VlmHelper::VlmHelper(const VlmConfig& vlm_cfg, const std::filesystem::path& devkit_dir,
  ...
  const bool is_spec_decode_draft =
      _vlm_cfg.lm_cfg.is_spec_decode() && _vlm_cfg.lm_cfg.speculative_decoding_cfg.value().is_draft;
  ...
  if (_vlm_cfg.gguf_file_name.empty()) {
    ...
    // Huggingface format.
    auto tokenizer_config_json =
        nlohmann::json::parse(std::ifstream(devkit_dir / "tokenizer_config.json"));
    ...
    _chat_template_ptr =
        std::make_unique<minja::chat_template>(chat_template_str, _bos_token, eos_token);
    _init_stop_token_ids(devkit_dir);
  }
}
...
void VlmHelper::_init_chat_template(const std::filesystem::path& devkit_dir,
  ...
  if (override_chat_template.has_value()) {
    ...
    auto chat_template_json = nlohmann::json::parse(std::ifstream(chat_template_json_file_name));
    chat_template_str = chat_template_json.at("chat_template");
  } else if (tokenizer_config_json.contains("chat_template") &&
    ...
    throw std::runtime_error(
        fmt::format("Model '{}' is missing a chat template. This runtime requires one of: "
...
void VlmHelper::_init_pad_token_id(const nlohmann::json& tokenizer_config_json) {
  if (!tokenizer_config_json.contains("pad_token") || tokenizer_config_json["pad_token"].is_null())
    return;
  _pad_token_id =
      _tokenizer_ptr->token_to_id(tokenizer_config_json["pad_token"].get<std::string>());
}
...
void VlmHelper::_init_image_processor(const std::filesystem::path& devkit_dir) {
  ...
  _image_processor_ptr =
      std::make_unique<ImageProcessor>(_vlm_cfg, do_pad_to_square, do_center_crop, interpolation,
                                       rescale_factor, std::move(image_mean), std::move(image_std));
}
