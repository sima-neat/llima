...
enum class DecodeCallbackType : uint32_t { TTFT = 1, TPS = 2, CACHE_FULL = 3, STOP = 4 };
...
public:
  ...
  TextStreamer(Tokenizer* tokenizer_ptr, std::optional<InfoCallback> info_callback = std::nullopt,
  ...
  void push(DecodeCallbackType type, uint32_t token_id, double duration, bool from_draft) {
