...
TextStreamer::TextStreamer(Tokenizer* tokenizer_ptr, std::optional<InfoCallback> info_callback,
                           std::optional<TextCallback> text_callback)
    : _tokenizer_ptr(tokenizer_ptr), _print_len(0), _is_enabled(true), _time_to_first_token(-1),
      ...
      _callback_finalize_text(
          text_callback.value_or([this](const std::string& text, bool stream_end, bool from_draft) {
...
void TextStreamer::end() {
  ...
  if (_time_to_first_token < 0) {
    ...
    // Calculate average.
    auto accum_time =
        std::accumulate(_time_to_next_token_vec.begin(), _time_to_next_token_vec.end(), 0.0);
    ...
    std::nth_element(_time_to_next_token_vec.begin(), _time_to_next_token_vec.begin() + mid_pos,
    ...
    if (_draft_token_count > 0) {
      const double accept_rate =
          100.0 * static_cast<double>(_draft_token_count) / static_cast<double>(num_gen_tokens);
      messages.emplace_back(fmt::format("Number of tokens contributed by EAGLE3: {} ({:.2f}%)",
                                        _draft_token_count, accept_rate));
    }
    messages.emplace_back(fmt::format("TTFT: {:.2f}s", _time_to_first_token));
    messages.emplace_back(fmt::format("TPS: max={:.2f}, avg={:.2f}, mid={:.2f}, min={:.2f}",
                                      max_tps, avg_tps, mid_tps, min_tps));
  } else {
...
bool TextStreamer::_ends_with_chinese_char(const std::string& text) {
  ...
  case 3:
    ...
    break;
  case 4:
    cp = (((data[0] & 0x07) << 18) | ((data[1] & 0x3F) << 12) | ((data[2] & 0x3F) << 6) |
          (data[3] & 0x3F));
    break;
  default:
    return false;
