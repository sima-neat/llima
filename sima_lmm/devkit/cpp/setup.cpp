...
static void setup_llama_cpp_logger() {
  ...
  llama_log_set(
      [](enum ggml_log_level level, const char* text, void* user_data) {
        if (!text || !llama_logger || !llama_logger->should_log(spdlog::level::debug))
          return;

        spdlog::level::level_enum msg_level;
        switch (level) {
        case GGML_LOG_LEVEL_ERROR:
          msg_level = spdlog::level::err;
          break;
...
void connect(const std::vector<std::string>& mla_rt_args,
