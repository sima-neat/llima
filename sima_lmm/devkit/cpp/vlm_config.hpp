...
inline void from_json(const nlohmann::json& j, VisionModelConfig& v) {
  ...
  v.num_spatial_patches = {static_cast<uint16_t>(v.image_sizes[0] / v.spatial_patch_size),
...
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE_WITH_DEFAULT(RopeScalingConfig, factor, low_freq_factor,
                                                high_freq_factor, original_max_position_embeddings,
                                                attention_factor, long_factor, short_factor,
                                                rope_type, mrope_section, mrope_interleaved)
...
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE_WITH_DEFAULT(RoPEConfig, rope_theta, rope_local_base_freq,
                                                rope_dimension_count, sliding_rope_dimension_count,
                                                rope_scaling)
...
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE_WITH_DEFAULT(AttentionBlockConfig, num_attention_heads,
                                                num_key_value_heads, head_dim, sliding_head_dim,
...
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE_WITH_DEFAULT(SpeculativeDecodingConfig, is_draft,
                                                speculative_budget)

struct LayerTypes : std::vector<std::string> {
...
struct LanguageModelConfig {
  ...
  uint32_t get_lm_head_output_size() const {
    if (speculative_decoding_cfg.has_value() && speculative_decoding_cfg.value().is_draft) {
      return draft_vocab_size;
    }
    return token_cfg.vocab_size;
  }
};
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE_WITH_DEFAULT(LanguageModelConfig, token_cfg, rope_cfg, attn_cfg,
                                                num_hidden_layers, hidden_size, lm_head_num_splits,
                                                lm_head_split_dim, layer_types, conv_L_cache,
...
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE_WITH_DEFAULT(PipelineConfig, system_prompt, chat_template,
                                                max_num_tokens, input_token_group_size,
                                                input_token_group_offsets, future_token_mask_size,
                                                return_logits, use_strided_kv_cache,
...
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE_WITH_DEFAULT(VlmConfig, model_name, model_type, vm_cfg, mm_cfg,
                                                lm_cfg, pipeline_cfg, vision_model_name,
