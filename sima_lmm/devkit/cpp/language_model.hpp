...
public:
  LanguageModel(std::filesystem::path model_path, std::set<uint32_t> stop_token_ids,
                std::optional<uint32_t> image_token_id, std::optional<uint32_t> pad_token_id,
  ...
  create_input_buffers(std::span<const uint32_t> input_token_ids);
  std::optional<std::vector<uint32_t>>
  run_model(std::span<const uint32_t> input_token_ids,
  ...
  uint32_t run_model_prefill(std::span<const uint32_t> input_token_ids, uint16_t num_cached_tokens,
                             std::optional<ChronoTimer> = std::nullopt);
  void run_model_decode(uint16_t num_input_tokens, uint32_t token_id);
  uint32_t run_model_once(uint16_t num_tokens, uint16_t token_idx, uint16_t num_input_tokens,
                          uint32_t token_id, std::vector<Eigen::bfloat16>* logits_ptr = nullptr);
  ...
  std::optional<std::vector<uint32_t>>
  run_model_speculative_decoding(LanguageModel& draft_lm, std::span<const uint32_t> input_token_ids,
                                 std::optional<uint16_t> override_max_num_tokens = std::nullopt,
                                 std::optional<ChronoTimer> timer_ttft = std::nullopt);
  void stop_model() {
    _is_running = false;
  ...
  TopkGenerateResult topk_generate(LanguageModel& target_lm, std::vector<uint32_t> input_ids,
                                   std::vector<Eigen::bfloat16> hidden_states,
                                   int num_cached_tokens, bool is_prefill);
  ...
  DraftForwardResult run_eagle3_draft_model(LanguageModel& target_lm,
                                            std::vector<Eigen::bfloat16> hidden_states,
  ...
  struct TargetVerifyResult {
    ...
    std::vector<std::vector<Eigen::bfloat16>>
        hidden_states; // 3 captures, each (num_tokens, hidden_size)
  };
  ...
  TargetVerifyResult run_eagle3_target_verify(std::vector<uint32_t> input_ids,
  ...
  TreeDecodingResult tree_decoding(std::vector<uint32_t> tree_candidates,
                                   std::vector<int32_t> tree_position_ids,
  ...
  InitTreeResult initialize_tree(LanguageModel& draft_lm, std::vector<uint32_t> input_ids,
                                 int num_cached_tokens, ChronoTimer timer_ttft);
  ...
  void compact_kv_after_accept(std::span<const uint16_t> select_indices, uint16_t prev_input_len);
  ...
  UpdateInferenceInputsResult
  update_inference_inputs(LanguageModel& draft_lm, std::vector<uint32_t> input_ids,
                          const std::vector<std::vector<int32_t>>& candidates,
                          size_t best_candidate, int32_t accept_length,
                          const std::vector<std::vector<int32_t>>& retrieve_indices,
...
private:
  ...
  void _define_model(const std::string& model_type, const LanguageModelMapKey& key,
  ...
  bool _uses_cpu_dequantized_embeddings() const {
    return _cfg.pipeline_cfg.quantize_embeddings && _cfg.vm_cfg.has_value() &&
           _cfg.mm_cfg.has_value();
  }
  uint16_t _prepare_state_checkpoints_for_prefill(uint16_t num_cached_tokens);
  void _save_state_checkpoint(size_t boundary_idx, uint16_t num_tokens, uint16_t valid_tokens);
  ...
  void _dequantize_embedding_row(uint32_t token_id, MLABuffer& dst, size_t dst_row = 0);
  std::vector<uint32_t> _get_per_layer_token_ids(std::span<const uint32_t> input_token_ids) const;
  void _upload_per_layer_embedding_rows(std::span<const uint32_t> token_ids, uint16_t num_tokens);
  void _load_per_layer_embeddings();
  void _compute_and_upload_per_layer_inputs_prefill(uint16_t num_tokens, uint16_t token_idx,
