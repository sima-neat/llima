...
class VlmHelper {
public:
  VlmHelper(const VlmConfig& vlm_cfg, const std::filesystem::path& devkit_dir,
  ...
  bool is_multimodal() const {
    return _vlm_cfg.is_multimodal();
  }
  bool support_image() const {
    return _vlm_cfg.support_image();
  }
  Tokenizer* get_tokenizer() {
    return _tokenizer_ptr.get();
  }
  std::set<uint32_t> get_stop_token_ids() const {
    return _stop_token_ids;
  }
  std::optional<uint32_t> get_image_token_id() const {
    return _image_token_id;
  }
  std::optional<uint32_t> get_pad_token_id() const {
    return _pad_token_id;
  }

private:
  void _init_chat_template(const std::filesystem::path& devkit_dir,
