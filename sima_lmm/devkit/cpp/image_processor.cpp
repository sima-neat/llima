...
ImageProcessor::ImageProcessor(const VlmConfig& vlm_cfg, bool do_pad_to_square, bool do_center_crop,
                               cv::InterpolationFlags interpolation, double rescale_factor,
                               std::vector<double> image_mean, std::vector<double> image_std)
    : _vlm_cfg(vlm_cfg), _target_height(_vlm_cfg.vm_cfg.value().image_sizes[0]),
      _target_width(_vlm_cfg.vm_cfg.value().image_sizes[1]), _do_pad_to_square(do_pad_to_square),
      _do_center_crop(do_center_crop), _interpolation(interpolation),
      _rescale_factor(rescale_factor), _image_mean(std::move(image_mean)),
...
std::vector<Eigen::bfloat16> ImageProcessor::_preprocess(cv::Mat image) {
  ...
  if (_do_center_crop) {
    ...
    if (orig_height < orig_width) {
      ...
      resize_width =
          static_cast<int>(static_cast<double>(orig_width * _target_height) / orig_height);
    } else {
      resize_width = _target_width;
      resize_height =
          static_cast<int>(static_cast<double>(orig_height * _target_width) / orig_width);
    }
  } else {
    resize_height = _target_height;
    resize_width = _target_width;
  ...
  cv::Mat transform_mat =
      (cv::Mat_<float>(3, 4) << 1.0 / _image_std[0], 0, 0, -_image_mean[0] / _image_std[0], 0,
       1.0 / _image_std[1], 0, -_image_mean[1] / _image_std[1], 0, 0, 1.0 / _image_std[2],
       -_image_mean[2] / _image_std[2]);
...
cv::Mat ImageProcessor::_pad_to_square(cv::Mat image) {
  ...
  cv::Scalar background_color(static_cast<uint8_t>(_image_mean[0] * 255),
...
std::vector<Eigen::bfloat16> ImageProcessor::_cast_and_patchify(cv::Mat image) {
  if (_vlm_cfg.model_type.starts_with("vlm-lfm2") ||
    ...
    Eigen::Map<ArrayXf> image_tensor_map(reinterpret_cast<float*>(image.data), cast_tensor.size());
...
std::vector<Eigen::bfloat16> ImageProcessor::_cast_and_patchify_lfm2_gemma4(cv::Mat image) {
  ...
  Eigen::TensorMap<Tensor3Df> image_tensor_map(reinterpret_cast<float*>(image.data), _target_height,
                                               _target_width, channel);
  Eigen::TensorMap<Tensor5Dbf> patched_tensor_map(patched_tensor.data(), num_patches_height,
                                                  num_patches_width, spatial_patch_size,
                                                  spatial_patch_size, channel);

  // Reshape dimensions.
  Eigen::array<Eigen::Index, 5> reshape_dims = {num_patches_height, spatial_patch_size,
                                                num_patches_width, spatial_patch_size, channel};
...
std::vector<Eigen::bfloat16> ImageProcessor::_cast_and_patchify_qwen(cv::Mat image) {
  ...
  // Reshape dimensions.
  Eigen::array<Eigen::Index, 9> reshape_dims = {1,
                                                1,
                                                num_grids_h,
                                                spatial_merge_size,
                                                spatial_patch_size,
  ...
  // Reshape and transpose.
  patched_tensor_map.device(get_eigen_device()) = (image_tensor_map.cast<Eigen::bfloat16>()
                                                       .reshape(reshape_dims)
                                                       .shuffle(transpose_axes)
                                                       .eval()
