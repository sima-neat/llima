...
class MLABuffer {
public:
  MLABuffer(std::string name, std::vector<size_t> shape, std::string dtype, bool align_last_dim);
  ~MLABuffer();
  void allocate();
  void try_allocate() {
    if (!_simaai_mem_ptr)
  ...
  void upload(const void* data, size_t data_begin = 0, size_t data_size = 0, bool flush = true);
  void download(void* data) const;
  uint32_t
  get_buf_addr_offset(const std::optional<std::vector<uint32_t>>& begin = std::nullopt) const;
  uint64_t get_buf_addr(const std::optional<std::vector<uint32_t>>& begin = std::nullopt) const;
  uint64_t get_buf_len(const std::optional<std::vector<uint32_t>>& shape = std::nullopt) const;
  ...
  const uint8_t get_elem_size() const {
    return _elem_size;
  }
  ...
  size_t get_num_elems() const {
    return _size / _elem_size;
  }
  void* get_virtual_addr() const {
    return _virtual_addr;
  }
  simaai_memory_t* get_simaai_memory() const {
    return _simaai_mem_ptr;
  }

  void print(std::ostream& s,
             const std::optional<std::vector<uint32_t>>& override_begin = std::nullopt,
             const std::optional<std::vector<uint32_t>>& override_shape = std::nullopt) const;

private:
  template <typename T>
  void print_content(std::ostream& s, uint32_t level, const T* curr_ptr,
                     const std::vector<uint32_t>& begin, const std::vector<uint32_t>& shape) const;
...
public:
  ...
  MLABufferSlice(MLABuffer* buf_ptr, std::vector<uint32_t> begins, std::vector<uint32_t> shapes);
  ~MLABufferSlice() {}

  MLABuffer* get_buf_ptr() const {
    return _buf_ptr;
  }
  uint64_t get_buf_addr() const;
  uint64_t get_buf_addr(const std::optional<std::vector<uint32_t>>& begins) const;
  auto get_buf_begins() const {
    return _begins;
  }
  auto get_buf_shapes() const {
    return _shapes;
  }
