...
struct WhisperConfig {
  ...
  uint32_t get_encoder_head_dim() const {
    return d_model / encoder_attention_heads;
  };
  uint32_t get_decoder_head_dim() const {
    return d_model / decoder_attention_heads;
  };
};

NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(WhisperConfig, model_name, model_type, d_model,
                                   encoder_attention_heads, encoder_layers, decoder_attention_heads,
                                   decoder_layers, max_source_positions, max_target_positions,
                                   num_mel_bins, vocab_size, decoder_use_future_token_mask,
                                   log_probe_enabled, language_token_ids, language_codes)
