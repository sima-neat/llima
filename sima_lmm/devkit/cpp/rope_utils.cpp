...
static Eigen::ArrayXf calc_rope_scaling_llama3(
    const Eigen::ArrayXf& inv_freq, const RopeScalingConfig& rope_scaling_cfg
) {
    ...
    Eigen::ArrayXf inv_freq_llama = (wavelen < high_freq_wavelen).select(
        inv_freq,
        (wavelen > low_freq_wavelen).select(
            inv_freq / factor,
            smoothed_inv_freq
        )
    );
...
static Eigen::ArrayXf calc_rope_scaling_longrope(
    const Eigen::ArrayXf& inv_freq,
    ...
    if (max_num_tokens > rope_scaling_cfg.original_max_position_embeddings) {
        ...
        auto long_factor = Eigen::Map<Eigen::ArrayXd>(
            rope_scaling_cfg.long_factor.value().data(),
        ...
        auto short_factor = Eigen::Map<Eigen::ArrayXd>(
            rope_scaling_cfg.short_factor.value().data(),
...
RopeTable calc_freq_real_imag(
    ...
    if (rope_type == "longrope") {
        double attention_factor = rope_scaling_cfg.attention_factor.value_or(1.0);
        re_float *= attention_factor;
        im_float *= attention_factor;
    }
...
RopeTable calc_mrope_with_image(
    ...
    while (true) {
        ...
        auto it = std::find(
            input_token_ids.begin() + token_idx, input_token_ids.end(), image_token_id
        );
        ...
        if (token_idx + mm_tokens_per_image > input_token_ids.size())
            throw std::runtime_error(
                "Not enough of input tokens for image " + std::to_string(image_idx)
            );

        // Fill the table for image tokens.
        if (mrope_interleaved) {
            assert(
                mrope_sections[0] >= mrope_sections[1] && mrope_sections[0] >= mrope_sections[2]
            );
