...
LanguageModel::LanguageModel(std::filesystem::path model_path, std::set<uint32_t> stop_token_ids,
                             std::optional<uint32_t> image_token_id,
                             std::optional<uint32_t> pad_token_id, TextStreamer& text_streamer)
    : BaseModel(model_path), _stop_token_ids(std::move(stop_token_ids)),
      _image_token_id(image_token_id), _pad_token_id(pad_token_id),
      _max_num_tokens(_cfg.pipeline_cfg.max_num_tokens), _text_streamer(text_streamer),
      _has_image_token(false), _is_running(false), _reloc_name(std::nullopt) {
  _use_group_token_models = (_cfg.pipeline_cfg.input_token_group_offsets.has_value() &&
  ...
  if (_use_group_token_models) {
    ...
    if (!conv_layer_indices.empty()) {
      ...
      conv_state.tail_bytes =
          static_cast<size_t>(conv_state.tail_len * conv_state.num_elems * conv_state.elem_size);
  ...
  if (_cfg.lm_cfg.is_spec_decode() && _cfg.lm_cfg.speculative_decoding_cfg.value().is_draft) {
    ...
    _eagle3_tree_mask_init.data = std::vector<std::vector<std::vector<std::vector<float>>>>(
        1, std::vector<std::vector<std::vector<float>>>(
               1, std::vector<std::vector<float>>(topk, std::vector<float>(topk, 0.0f))));
    for (int i = 0; i < topk; ++i)
      _eagle3_tree_mask_init.data[0][0][i][i] = 1.0f;
...
std::vector<std::map<uint8_t, MLABufferSlice>>
LanguageModel::create_input_buffers(std::span<const uint32_t> input_token_ids) {
  // Log.
  ...
  define_buffer("input_embeds", {num_padded_input_tokens, _cfg.lm_cfg.hidden_size},
                (_cfg.vm_cfg.has_value() && _cfg.mm_cfg.has_value()) ? "bfloat16"
  ...
  if (_cfg.vm_cfg.has_value()) {
    ...
    do {
      auto it =
          std::find(input_token_ids.begin() + token_idx, input_token_ids.end(), _image_token_id);
      ...
      ofm_map.emplace(std::piecewise_construct, std::forward_as_tuple(0),
                      std::forward_as_tuple(
                          &get_buffer("input_embeds"), std::vector<uint32_t>{token_idx, 0},
                          std::vector<uint32_t>{mm_tokens_per_image, _cfg.lm_cfg.hidden_size}));
      for (size_t i = 0; i < _cfg.vm_cfg.value().deepstack_visual_indexes.size(); ++i) {
        ofm_map.emplace(std::piecewise_construct, std::forward_as_tuple(i + 1),
                        std::forward_as_tuple(
...
LanguageModel::run_model(std::span<const uint32_t> input_token_ids,
  ...
  if (input_token_ids.size() > _max_num_tokens) {
    ...
    if (_stop_token_ids.contains(token_id)) {
      ...
      output_token_ids = std::vector<uint32_t>(_cached_token_ids.begin() + output_token_id_begin,
                                               _cached_token_ids.end());
    }
  }
...
uint32_t LanguageModel::run_model_prefill(std::span<const uint32_t> input_token_ids,
  ...
  if (num_input_tokens == num_cached_tokens) {
    ...
    while (token_idx < num_input_tokens) {
      if (it != offsets.end()) {
        ...
        token_idx = *it;
        last_group_valid_tokens =
            std::min(num_input_tokens, static_cast<uint16_t>(token_idx + num_tokens)) - token_idx;
        if (_uses_per_layer_inputs()) {
          _compute_and_upload_per_layer_inputs_prefill(num_tokens, token_idx, num_input_tokens);
        }
      ...
      if (!_is_running.load(std::memory_order_relaxed)) {
        _notify_interrupt();
        _cached_token_ids.assign(input_token_ids.begin(), input_token_ids.begin() + token_idx);
        return next_token_id;
      }
    }
    if (!_cached_states.empty() && last_group_valid_tokens > 0 &&
    ...
    for (token_idx = num_cached_tokens; token_idx < num_input_tokens; ++token_idx) {
      ...
      if (!_is_running.load(std::memory_order_relaxed)) {
        _notify_interrupt();
        _cached_token_ids.assign(input_token_ids.begin(), input_token_ids.begin() + token_idx + 1);
        return next_token_id;
      }
    }
  }
...
void LanguageModel::run_model_decode(uint16_t num_input_tokens, uint32_t token_id) {
...
uint32_t LanguageModel::run_model_once(uint16_t num_tokens, uint16_t token_idx,
    ...
    if (!use_input_tokens) {
      ...
      const uint32_t per_layer_token_id =
          (_image_token_id.has_value() && token_id == _image_token_id.value())
              ? _pad_token_id.value()
              : token_id;
      const size_t shard_idx = (per_layer_token_id / _per_layer_embedding_rows_per_shard);
      const size_t row_in_shard = (per_layer_token_id % _per_layer_embedding_rows_per_shard);
      auto* shard = _per_layer_embedding_shards[shard_idx];
      per_layer_ifm_map.emplace(
          std::piecewise_construct, std::forward_as_tuple(0),
          std::forward_as_tuple(
              shard, std::vector<uint32_t>{static_cast<uint32_t>(row_in_shard), 0},
              std::vector<uint32_t>{1, static_cast<uint32_t>(shard->get_shape().back())}));
    }
    per_layer_ifm_map.emplace(
        std::piecewise_construct, std::forward_as_tuple(1),
        std::forward_as_tuple(
            normal_input_buf, std::vector<uint32_t>{normal_input_row, 0},
  ...
  for (uint8_t layer_idx = 0; layer_idx < _cfg.lm_cfg.num_hidden_layers; ++layer_idx) {
    ...
    if (layer_idx == 0) {
      ifm_map.emplace(std::piecewise_construct, std::forward_as_tuple(0),
                      std::forward_as_tuple(
                          normal_input_buf, std::vector<uint32_t>{normal_input_row, 0},
                          std::vector<uint32_t>{normal_input_num_tokens, _cfg.lm_cfg.hidden_size}));
    }
    if (_cfg.lm_cfg.layer_types[layer_idx] == "full_attention" ||
        _cfg.lm_cfg.layer_types[layer_idx] == "sliding_attention") {
      _pre_model_map.at(model_key).add_to_queue(&ifm_map);

      if (num_input_tokens > next_token_idx && layer_idx == _cfg.lm_cfg.num_hidden_layers - 1) {
        break;
      }
      ...
      const bool is_draft =
          _cfg.lm_cfg.is_spec_decode() && _cfg.lm_cfg.speculative_decoding_cfg.value().is_draft;
      ...
      const bool use_single_post_for_target_group =
          (_cfg.lm_cfg.is_spec_decode() && !is_draft && num_tokens != single_num_tokens &&
      ...
      if (num_tokens > 1 && layer_idx == _cfg.lm_cfg.num_hidden_layers - 1 &&
        ...
        ifm_map.emplace(
            ...
            std::forward_as_tuple(
                nullptr,
                std::vector<uint32_t>{static_cast<uint32_t>(num_input_tokens - 1 - token_idx), 0},
        ...
        ifm_map.emplace(
            ...
            std::forward_as_tuple(
                ...
                std::vector<uint32_t>{static_cast<uint32_t>(num_input_tokens - 1 - token_idx), 0},
                std::vector<uint32_t>{
                    1, _cfg.lm_cfg.attn_cfg.get_q_size(_cfg.lm_cfg.layer_types[layer_idx])}));
      }
      if (use_single_post_for_target_group) {
        const uint32_t last_valid_row = num_input_tokens - 1 - token_idx;
        const uint32_t slice_start =
            last_valid_row >= single_num_tokens - 1 ? last_valid_row - (single_num_tokens - 1) : 0;

        ifm_map.clear();
        ifm_map.emplace(std::piecewise_construct, std::forward_as_tuple(0),
                        std::forward_as_tuple(
                            nullptr, std::vector<uint32_t>{slice_start, 0},
                            std::vector<uint32_t>{single_num_tokens, _cfg.lm_cfg.hidden_size}));
        ifm_map.emplace(
            std::piecewise_construct, std::forward_as_tuple(1),
            std::forward_as_tuple(
                nullptr, std::vector<uint32_t>{slice_start, 0},
                std::vector<uint32_t>{single_num_tokens, _cfg.lm_cfg.attn_cfg.get_q_size(
                                                             _cfg.lm_cfg.layer_types[layer_idx])}));
        uint8_t post_ifm_idx = 2;
        if (_uses_per_layer_inputs()) {
          const uint32_t layer_row_offset = static_cast<uint32_t>(layer_idx) * num_tokens;
          ifm_map.emplace(std::piecewise_construct, std::forward_as_tuple(post_ifm_idx++),
                          std::forward_as_tuple(
                              nullptr, std::vector<uint32_t>{layer_row_offset + slice_start, 0},
                              std::vector<uint32_t>{single_num_tokens,
        ...
        if (_cfg.vm_cfg.has_value() &&
          ...
          ifm_map.emplace(std::piecewise_construct, std::forward_as_tuple(post_ifm_idx),
                          std::forward_as_tuple(
                              nullptr, std::vector<uint32_t>{token_idx + slice_start, 0},
                              std::vector<uint32_t>{
                                  single_num_tokens,
      ...
      if (_uses_per_layer_inputs() && num_tokens > 1 &&
          layer_idx == _cfg.lm_cfg.num_hidden_layers - 1 && !_cfg.lm_cfg.is_spec_decode()) {
      ...
      // into FC fusion.
      if (_cfg.lm_cfg.is_spec_decode() && layer_idx < _cfg.lm_cfg.num_hidden_layers - 1) {
      ...
      throw std::runtime_error(std::string("Unsupported layer type: ") +
                               _cfg.lm_cfg.layer_types[layer_idx]);
    }
  }
  ...
  if (num_input_tokens <= next_token_idx) {
    if (_cfg.lm_cfg.is_spec_decode()) {
      ...
      const uint16_t single_num_tokens = _cfg.lm_cfg.get_single_num_tokens();
      const uint32_t last_valid_row = num_input_tokens - 1 - token_idx;
      const bool used_single_post_for_target_group = (!is_draft && num_tokens != single_num_tokens);
      const uint32_t slice_start =
          (used_single_post_for_target_group && last_valid_row >= single_num_tokens - 1)
              ? last_valid_row - (single_num_tokens - 1)
              : 0;
      const uint32_t row = last_valid_row - slice_start;
      const uint16_t logits_num_tokens =
          used_single_post_for_target_group ? single_num_tokens : num_tokens;
      const uint32_t vocab_size = _cfg.lm_cfg.get_lm_head_output_size();

      if (_cfg.lm_cfg.lm_head_num_splits == 1) {
        MLABuffer* buf_ptr = &get_buffer(fmt::format("n{}_buffer4", logits_num_tokens));
        buf_ptr->invalidate_cache();
        ...
        for (uint32_t split_begin = 0; split_begin < vocab_size; split_begin += split_dim, ++i) {
          ...
          MLABuffer* buf_ptr = &get_buffer(fmt::format("n{}_lm_split{}", logits_num_tokens, i));
          buf_ptr->invalidate_cache();
          auto* ptr = reinterpret_cast<Eigen::bfloat16*>(buf_ptr->get_virtual_addr());
...
void LanguageModel::_initialize() {
  ...
  auto rope_table = calc_freq_real_imag(
      _cfg.pipeline_cfg.max_num_tokens, _cfg.lm_cfg.rope_cfg.rope_scaling.rope_type,
      _cfg.lm_cfg.rope_cfg.rope_theta,
      _cfg.lm_cfg.rope_cfg.get_rope_dimension_count("full_attention"),
      _cfg.lm_cfg.attn_cfg.get_head_dim("full_attention"), _cfg.lm_cfg.rope_cfg.rope_scaling);
  ...
  if (_cfg.lm_cfg.attn_cfg.swa_enable) {
    rope_table = calc_freq_real_imag(
        _cfg.pipeline_cfg.max_num_tokens, "default", _cfg.lm_cfg.rope_cfg.rope_local_base_freq,
        _cfg.lm_cfg.rope_cfg.get_rope_dimension_count("sliding_attention"),
        _cfg.lm_cfg.attn_cfg.get_head_dim("sliding_attention"), _cfg.lm_cfg.rope_cfg.rope_scaling);
  ...
  if (_cfg.pipeline_cfg.future_token_mask_size > 1) {
    std::vector<Eigen::bfloat16> future_token_mask(_cfg.pipeline_cfg.future_token_mask_size - 1,
    ...
    future_token_mask_buf.upload(future_token_mask.data(), _cfg.pipeline_cfg.max_num_tokens * 2,
                                 future_token_mask.size() * 2, true);
  }
...
// Restore the latest checkpoint <= num_cached_tokens so grouped prefill can resume from a saved
// position.
uint16_t LanguageModel::_prepare_state_checkpoints_for_prefill(uint16_t num_cached_tokens) {
  ...
  auto it = std::upper_bound(_checkpoint_boundaries.begin(), _checkpoint_boundaries.end(),
                             num_cached_tokens);
  ...
  for (auto& state : _cached_states) {
    const size_t dst_offset = static_cast<size_t>(tail_begin * state.num_elems * state.elem_size);
    for (size_t layer_slot = 0; layer_slot < state.layer_indices.size(); ++layer_slot) {
      ...
      buf.upload(checkpoints[requested_boundary].data(), dst_offset, state.tail_bytes, true);
    }
  }
  return restored_token_count;
}

void LanguageModel::_save_state_checkpoint(size_t boundary_idx, uint16_t num_tokens,
  ...
  const uint32_t tail_row_offset =
      (num_tokens > 1) ? static_cast<uint32_t>(valid_tokens - 1)
  ...
  for (auto& state : _cached_states) {
    ...
    for (size_t layer_slot = 0; layer_slot < state.layer_indices.size(); ++layer_slot) {
      ...
      std::memcpy(state.checkpoints[layer_slot][boundary_idx].data(), ptr + src_offset_bytes,
                  state.tail_bytes);
    }
  }
}
...
void LanguageModel::_define_buffer_freq_table(const std::string& name,
  ...
  define_buffer(fmt::format("{}_freq_real", name),
                {_cfg.pipeline_cfg.max_num_tokens, rope_dimension_count / 2});
  define_buffer(fmt::format("{}_freq_imag", name),
                {_cfg.pipeline_cfg.max_num_tokens, rope_dimension_count / 2});
}
...
void LanguageModel::_define_buffers() {
  ...
  const bool is_draft =
      _cfg.lm_cfg.is_spec_decode() && _cfg.lm_cfg.speculative_decoding_cfg.value().is_draft;
  if (!is_draft) {
    define_buffer("embeddings", {_cfg.lm_cfg.token_cfg.vocab_size, _cfg.lm_cfg.hidden_size},
  ...
  const uint16_t conv_working_len =
      _cfg.pipeline_cfg.input_token_group_size + _cfg.lm_cfg.conv_L_cache - 2;
  ...
  for (uint8_t i = 0; i < _cfg.lm_cfg.num_hidden_layers; ++i) {
    if (_cfg.lm_cfg.layer_types[i] == "conv") {
      ...
      if (_cfg.pipeline_cfg.use_strided_kv_cache) {
        cache_shape = {_cfg.lm_cfg.attn_cfg.num_key_value_heads, _cfg.pipeline_cfg.max_num_tokens,
                       _cfg.lm_cfg.attn_cfg.get_head_dim(_cfg.lm_cfg.layer_types[i])};
      } else {
        cache_shape = {_cfg.pipeline_cfg.max_num_tokens,
      ...
      if (!_cfg.lm_cfg.is_kv_shared_layer(i)) {
        ...
        if (_cfg.pipeline_cfg.quantize_kv_cache) {
          ...
          std::vector<size_t> scale_shape = {_cfg.lm_cfg.attn_cfg.num_key_value_heads,
                                             _cfg.pipeline_cfg.max_num_tokens, 8};
  ...
  if (_cfg.vm_cfg.has_value()) {
    for (size_t i = 0; i < _cfg.vm_cfg.value().deepstack_visual_indexes.size(); ++i) {
      define_buffer(fmt::format("deepstack_feature_l{}_cache", i),
  ...
  if (_cfg.pipeline_cfg.future_token_mask_size > 1) {
    if (_cfg.lm_cfg.is_spec_decode()) {
      define_buffer("future_token_mask",
                    {_cfg.lm_cfg.get_single_num_tokens(), _cfg.pipeline_cfg.max_num_tokens});
    } else {
      uint32_t full_token_mask_size = round_up_to_row(_cfg.pipeline_cfg.max_num_tokens +
  ...
  for (const auto& num_tokens : num_tokens_vec) {
    ...
    define_buffer(fmt::format("n{}_buffer1", num_tokens), {num_tokens, _cfg.lm_cfg.hidden_size});
    ...
    define_buffer(fmt::format("n{}_buffer3", num_tokens),
    ...
    if (is_draft) {
      define_buffer(fmt::format("n{}_buffer1a", num_tokens), {num_tokens, _cfg.lm_cfg.hidden_size});
      define_buffer(fmt::format("n{}_buffer5", num_tokens), {num_tokens, _cfg.lm_cfg.hidden_size});
      define_buffer(fmt::format("fc_n{}_input", num_tokens),
                    {num_tokens, _cfg.lm_cfg.hidden_size * 3});
      define_buffer(fmt::format("fc_n{}_output", num_tokens),
                    {num_tokens, _cfg.lm_cfg.hidden_size});
    }
  }

  if (_uses_per_layer_inputs()) {
    const std::string dtype = _cfg.pipeline_cfg.quantize_embeddings ? "int8" : "bfloat16";
    ...
    const size_t elem_size =
        _cfg.pipeline_cfg.quantize_embeddings ? sizeof(int8_t) : sizeof(Eigen::bfloat16);
    _per_layer_embedding_rows_per_shard =
        std::max<size_t>(1, PER_LAYER_EMBEDDING_MAX_SHARD_SIZE / (out_dim * elem_size));
    const size_t vocab_size = _cfg.lm_cfg.token_cfg.vocab_size;
    const size_t num_shards = (vocab_size + _per_layer_embedding_rows_per_shard - 1) /
                              _per_layer_embedding_rows_per_shard;
    ...
    for (size_t shard_idx = 0; shard_idx < num_shards; ++shard_idx) {
      ...
      const size_t num_rows = std::min(_per_layer_embedding_rows_per_shard, vocab_size - row_begin);
      const auto name = fmt::format("per_layer_embeddings_s{}", shard_idx);
      define_buffer(name, {num_rows, out_dim}, dtype);
    ...
    if (_use_group_token_models) {
      define_buffer(
          ...
          {_cfg.pipeline_cfg.input_token_group_size, out_dim}, dtype);
    }
  }
  ...
  if (!_cfg.lm_cfg.is_spec_decode()) {
    ...
    for (const auto& num_tokens : num_tokens_vec) {
      ...
      if (_cfg.lm_cfg.lm_head_num_splits == 1 && !_cfg.pipeline_cfg.return_logits) {
        define_buffer(fmt::format("n{}_buffer4", num_tokens), {num_tokens, 1}, "int32");
      } else if (_cfg.lm_cfg.lm_head_num_splits == 1) {
        define_buffer(fmt::format("n{}_buffer4", num_tokens), {num_tokens, lm_head_output_size});
      } else {
        uint32_t i = 0;
        for (uint32_t split_begin = 0; split_begin < lm_head_output_size;
          ...
          define_buffer(fmt::format("n{}_lm_split{}", num_tokens, i), {num_tokens, split_size});
        }
      }
    }
  }
...
void LanguageModel::_define_model(const std::string& model_type, const LanguageModelMapKey& key,
...
void LanguageModel::_define_attn_models_iter(uint16_t num_tokens, uint16_t token_idx,
                                             uint8_t layer_idx) {
  LanguageModelMapKey model_key{num_tokens, layer_idx, token_idx};
  ...
  if (layer_type == "sliding_attention") {
    ...
    cache_token_idx_begin = std::max(
        0, token_idx + num_tokens - static_cast<int>(_cfg.lm_cfg.attn_cfg.sliding_window.value()));
  } else {
    freq_prefix = "global";
  ...
  if (_cfg.pipeline_cfg.use_strided_kv_cache) {
    ...
    pre_kv_cache_shape = {_cfg.lm_cfg.attn_cfg.num_key_value_heads,
  ...
  if (layer_idx) {
    pre_ifms.emplace_back(MLABufferSlice{&get_buffer(fmt::format("n{}_buffer1", num_tokens))});
    if (is_draft) {
      pre_ifms.emplace_back(MLABufferSlice{&get_buffer(fmt::format("n{}_buffer1a", num_tokens))});
    }
  } else {
  ...
  pre_ifms.emplace_back(
      MLABufferSlice{&get_buffer(buf_name),
                     {token_idx, 0},
  ...
  pre_ofms.emplace_back(MLABufferSlice{&get_buffer(fmt::format("n{}_buffer2", num_tokens)),
                                       {0, 0, 0},
                                       {_cfg.lm_cfg.attn_cfg.num_attention_heads, num_tokens,
                                        _cfg.lm_cfg.attn_cfg.get_head_dim(layer_type)}});
  if (!_cfg.lm_cfg.is_kv_shared_layer(layer_idx)) {
    pre_ofms.emplace_back(MLABufferSlice(&get_buffer(fmt::format("cache_key_l{}", layer_idx)),
                                         pre_kv_cache_offset, pre_kv_cache_shape));
    if (_cfg.pipeline_cfg.quantize_kv_cache) {
      pre_ofms.emplace_back(MLABufferSlice(
          &get_buffer(fmt::format("cache_key_scale_l{}", layer_idx)), {0, token_idx, 0},
          {_cfg.lm_cfg.attn_cfg.num_key_value_heads, _cfg.pipeline_cfg.max_num_tokens, 8}));
    }
    pre_ofms.emplace_back(MLABufferSlice(&get_buffer(fmt::format("cache_val_l{}", layer_idx)),
                                         pre_kv_cache_offset, pre_kv_cache_shape));
    if (_cfg.pipeline_cfg.quantize_kv_cache) {
      pre_ofms.emplace_back(MLABufferSlice(
          &get_buffer(fmt::format("cache_val_scale_l{}", layer_idx)), {0, token_idx, 0},
          {_cfg.lm_cfg.attn_cfg.num_key_value_heads, _cfg.pipeline_cfg.max_num_tokens, 8}));
    }
  }
  _define_model("pre", model_key, _get_elf_path_pre(num_tokens, layer_idx), pre_ifms, pre_ofms);
  ...
  if (num_tokens == single_num_tokens && _cfg.pipeline_cfg.future_token_mask_size > 1) {
    ...
    aligned_eff_token_idx =
        std::min(round_up_to(eff_num_cached_tokens, _cfg.pipeline_cfg.future_token_mask_size) - 1,
  ...
  if (_cfg.pipeline_cfg.use_strided_kv_cache) {
    ...
    cache_kv_cache_shape = {_cfg.lm_cfg.attn_cfg.num_key_value_heads,
                            _cfg.pipeline_cfg.max_num_tokens,
                            _cfg.lm_cfg.attn_cfg.get_head_dim(kv_layer_type)};
  } else {
    cache_kv_cache_offset = {cache_token_idx_begin, 0};
    cache_kv_cache_shape = {aligned_eff_num_cached_tokens,
  ...
  std::vector<MLABufferSlice> cache_ifms{
      MLABufferSlice{&get_buffer(fmt::format("n{}_buffer2", num_tokens)),
                     ...
                     {_cfg.lm_cfg.attn_cfg.num_attention_heads, num_tokens,
                      _cfg.lm_cfg.attn_cfg.get_head_dim(layer_type)}},
      MLABufferSlice{&get_buffer(fmt::format("cache_key_l{}", kv_source_layer)),
                     cache_kv_cache_offset, cache_kv_cache_shape},
  };
  if (_cfg.pipeline_cfg.quantize_kv_cache) {
    cache_ifms.emplace_back(MLABufferSlice{
  ...
  if (num_tokens == single_num_tokens && _cfg.pipeline_cfg.future_token_mask_size > 1) {
    if (_cfg.lm_cfg.is_spec_decode()) {
      ...
      cache_ifms.emplace_back(MLABufferSlice{&get_buffer("future_token_mask"),
                                             {0, cache_token_idx_begin},
                                             {num_tokens, aligned_eff_num_cached_tokens}});
    } else {
      cache_ifms.emplace_back(
          MLABufferSlice{&get_buffer("future_token_mask"),
                         {(uint32_t)_cfg.pipeline_cfg.max_num_tokens - eff_num_cached_tokens},
                         {aligned_eff_num_cached_tokens}});
    }
  }
  cache_ifms.emplace_back(MLABufferSlice{&get_buffer(fmt::format("cache_val_l{}", kv_source_layer)),
                                         cache_kv_cache_offset, cache_kv_cache_shape});
  if (_cfg.pipeline_cfg.quantize_kv_cache) {
    cache_ifms.emplace_back(MLABufferSlice{
  ...
  _define_model("cache", model_key,
                _get_elf_path_cache(num_tokens, aligned_eff_token_idx, layer_idx), cache_ifms,
                cache_ofms);
  ...
  if (use_single_post_for_target_group) {
    post_ifms[0] = MLABufferSlice{
        post_ifms[0].get_buf_ptr(), {0, 0}, {single_num_tokens, _cfg.lm_cfg.hidden_size}};
    post_ifms[1] = MLABufferSlice{post_ifms[1].get_buf_ptr(),
                                  {0, 0},
  ...
  if (_uses_per_layer_inputs()) {
    post_ifms.emplace_back(MLABufferSlice{
        ...
        std::vector<uint32_t>{use_single_post_for_target_group ? single_num_tokens : num_tokens,
                              _cfg.lm_cfg.hidden_size_per_layer_input}});
  }
  if (_cfg.vm_cfg.has_value() && layer_idx < _cfg.vm_cfg.value().deepstack_visual_indexes.size() &&
      num_tokens > 1) {
  ...
  if (layer_idx < _cfg.lm_cfg.num_hidden_layers - 1) {
    post_ofms.emplace_back(MLABufferSlice{&get_buffer(fmt::format("n{}_buffer1", num_tokens))});
      ...
      if (_cfg.lm_cfg.is_spec_decode()) {
        ...
        for (uint32_t split_begin = 0; split_begin < lm_head_output_size;
          ...
          post_ofms.emplace_back(
              MLABufferSlice{&get_buffer(fmt::format("n{}_lm_split{}", post_num_tokens, i))});
        }
      } else {
        for (uint32_t split_begin = 0; split_begin < lm_head_output_size;
    ...
    if (is_draft) {
      post_ofms.emplace_back(MLABufferSlice{&get_buffer(fmt::format("n{}_buffer5", num_tokens))});
    }
  }
...
void LanguageModel::_define_conv_models_iter(uint16_t num_tokens, uint8_t layer_idx) {
  ...
  if (layer_idx) {
    conv_ifms.emplace_back(MLABufferSlice{&get_buffer(fmt::format("n{}_buffer1", num_tokens))});
  } else {
    conv_ifms.emplace_back(MLABufferSlice{});
  }
  conv_ifms.emplace_back(
      MLABufferSlice{&get_buffer(fmt::format("conv_cache_history_l{}", layer_idx)),
                     {tail_begin, 0},
  ...
  conv_ofms.emplace_back(
      MLABufferSlice(&get_buffer(fmt::format("conv_cache_history_l{}", layer_idx)),
                     ...
                     {static_cast<uint32_t>(num_tokens + tail_size - 1), _cfg.lm_cfg.hidden_size}));
  _define_model("conv", model_key, _get_elf_path_conv(num_tokens, layer_idx), conv_ifms, conv_ofms);
  ...
  _define_model("conv_final", model_key, _get_elf_path_conv_final(layer_idx), conv_final_ifms,
                conv_final_ofms);
}
...
void LanguageModel::_define_draft_fc_models() {
  ...
  for (const auto& num_tokens : num_tokens_vec) {
    auto elf_path = _elf_dir / fmt::format("{}_n{}_draft_fc_stage1_mla.elf",
    ...
    _fc_model_map.emplace(num_tokens, MLAModelWithBuffer(elf_path, ifms, ofms));
  }
}

void LanguageModel::compact_kv_after_accept(std::span<const uint16_t> select_indices,
  ...
  for (uint8_t layer_idx = 0; layer_idx < num_layers; ++layer_idx) {
    ...
    const size_t head_dim_bytes = static_cast<size_t>(head_dim) * sizeof(Eigen::bfloat16);
    const size_t head_stride_bytes = static_cast<size_t>(max_num_tokens) * head_dim_bytes;
    ...
    for (const std::string& kind : {"cache_key_l", "cache_val_l"}) {
      ...
      for (uint32_t h = 0; h < num_kv_heads; ++h) {
        ...
        for (size_t i = 0; i < n; ++i) {
          std::memcpy(tmp.data() + i * head_dim_bytes,
        ...
        for (size_t i = 0; i < n; ++i) {
          std::memcpy(head_base + (static_cast<size_t>(prev_input_len) + i) * head_dim_bytes,
                      tmp.data() + i * head_dim_bytes, head_dim_bytes);
        }
      }
...
void LanguageModel::set_reloc(const std::string& reloc_name) {
  ...
  for (const auto& [reloc_map_type, reloc_addr_map] : reloc_addr_maps) {
    ...
    if (reloc_map_type.model_type == "pre") {
      ...
      auto msg = fmt::format("Relocate data for {} is not supported", reloc_map_type.model_type);
      throw std::runtime_error(msg);
    }
...
std::set<uint32_t>
LanguageModel::set_stop_token_ids(std::optional<std::set<uint32_t>> stop_token_ids) {
  auto original_stop_token_ids = _stop_token_ids;
  if (stop_token_ids.has_value()) {
    _stop_token_ids = stop_token_ids.value();
  }
  _logger->info("Setting stop_token_ids: {} -> {}", original_stop_token_ids, _stop_token_ids);
  return original_stop_token_ids;
}

std::filesystem::path LanguageModel::_get_elf_path_pre(uint16_t num_tokens, uint8_t layer_idx) {
  auto elf_file_name = fmt::format("{}_n{}_pre_layer{}_stage1_mla.elf", _cfg.language_model_name,
                                   num_tokens, layer_idx);
...
std::filesystem::path LanguageModel::_get_elf_path_cache(uint16_t num_tokens, uint16_t token_idx,
  ...
  auto elf_file_name = fmt::format("{}_n{}_{}_token{}_stage1_mla.elf", _cfg.language_model_name,
                                   num_tokens, cache_name, token_idx);
  return _elf_dir / elf_file_name;
}

std::filesystem::path LanguageModel::_get_elf_path_post(uint16_t num_tokens, uint8_t layer_idx) {
  auto elf_file_name = fmt::format("{}_n{}_post_layer{}_stage1_mla.elf", _cfg.language_model_name,
                                   num_tokens, layer_idx);
  return _elf_dir / elf_file_name;
}

std::filesystem::path LanguageModel::_get_elf_path_conv(uint16_t num_tokens, uint8_t layer_idx) {
  auto elf_file_name = fmt::format("{}_n{}_layer{}_conv_stage1_mla.elf", _cfg.language_model_name,
                                   num_tokens, layer_idx);
  return _elf_dir / elf_file_name;
}

std::filesystem::path LanguageModel::_get_elf_path_conv_final(uint8_t layer_idx) {
  auto elf_file_name = fmt::format("{}_n1_post_layer{}_conv_final_stage1_mla.elf",
                                   _cfg.language_model_name, layer_idx);
  return _elf_dir / elf_file_name;
}

std::filesystem::path LanguageModel::_get_elf_path_per_layer(uint16_t num_tokens) {
  auto elf_file_name =
      fmt::format("{}_n{}_per_layer_stage1_mla.elf", _cfg.language_model_name, num_tokens);
  return _elf_dir / elf_file_name;
}

void LanguageModel::_dequantize_embedding_row(uint32_t token_id, MLABuffer& dst, size_t dst_row) {
  ...
  auto* dst_row_ptr =
      reinterpret_cast<Eigen::bfloat16*>(reinterpret_cast<uint8_t*>(dst.get_virtual_addr()) +
...
uint16_t LanguageModel::_set_input_text_embeds(std::span<const uint32_t> input_token_ids) {
  ...
  const bool dequantize = embeddings_buf.get_dtype() == "int8" && buf.get_dtype() == "bfloat16";
  uint32_t token_idx = 0;
  ...
  while (token_idx < num_input_tokens) {
    ...
    if (_image_token_id.has_value() && token_id == _image_token_id.value()) {
      ...
      if (dequantize) {
        ...
        buf.upload(embeddings_ptr + token_id * embeddings_row_size, token_idx * input_row_size,
                   input_row_size, false);
      }
      if (num_images == 0 && token_idx < _cached_token_ids.size() &&
  ...
  if (_cfg.lm_cfg.rope_cfg.rope_scaling.rope_type == "mrope" &&
    ...
    auto rope_table =
        calc_mrope_with_image(_cfg, _master_rope_table, _image_token_id.value(), input_token_ids);
    get_buffer("global_freq_real").upload(rope_table.re.data());
    get_buffer("global_freq_imag").upload(rope_table.im.data());
  }
  _has_image_token = num_images > 0;
...
std::vector<uint32_t>
LanguageModel::_get_per_layer_token_ids(std::span<const uint32_t> input_token_ids) const {
...
void LanguageModel::_load_per_layer_embeddings() {
  const auto file_name = (_devkit_dir / (_cfg.language_model_name + "_per_layer_embeddings.bin"));
  const size_t vocab_size = _cfg.lm_cfg.token_cfg.vocab_size;
  const size_t out_dim =
      static_cast<size_t>(_cfg.lm_cfg.num_hidden_layers) * _cfg.lm_cfg.hidden_size_per_layer_input;
  const size_t elem_size =
      _cfg.pipeline_cfg.quantize_embeddings ? sizeof(int8_t) : sizeof(Eigen::bfloat16);
  ...
  if (actual_size != expected_size) {
    throw std::runtime_error(fmt::format("Invalid size for {}: expected {} bytes, got {}",
  ...
  for (auto* shard : _per_layer_embedding_shards) {
    if (shard->get_buf_len() != shard->get_shape()[0] * token_row_size) {
      throw std::runtime_error(fmt::format(
          "Per-layer embedding rows require unsupported MLA padding: {}", shard->get_name()));
    }
    shard->load_stream(stream);
  }
}

void LanguageModel::_upload_per_layer_embedding_rows(std::span<const uint32_t> token_ids,
                                                     uint16_t num_tokens) {
  if (!_uses_per_layer_inputs())
    return;
...
void LanguageModel::_compute_and_upload_per_layer_inputs_prefill(uint16_t num_tokens,
                                                                 uint16_t token_idx,
