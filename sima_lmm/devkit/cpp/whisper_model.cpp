...
ArrayXXbf WhisperPreprocessor::preprocess(const std::filesystem::path& audio_file_name) {
  auto audio_tensor = _load_audio_ffmpeg(audio_file_name);
  auto log_spec = _log_mel_spectrogram(audio_tensor);
  return log_spec;
}

ArrayXXbf WhisperPreprocessor::preprocess_pcm(std::span<const float> pcm, uint32_t sample_rate) {
...
ArrayXf WhisperPreprocessor::_load_audio_ffmpeg(const std::filesystem::path& audio_file_name) {
  ...
  if (stream_idx < 0)
    throw std::runtime_error("LibAv: No audio stream");

  AVCodecParameters* params = fmt_ctx->streams[stream_idx]->codecpar;
  const AVCodec* codec = avcodec_find_decoder(params->codec_id);
  if (!codec)
    throw std::runtime_error("LibAv: Decoder not found");

  // 3. Setup Codec
  ScopedPtr<AVCodecContext> codec_ctx(avcodec_alloc_context3(codec));
  if (!codec_ctx)
    throw std::runtime_error("LibAv: Codec alloc failed");
  ...
  int ret =
      swr_alloc_set_opts2(&swr_raw, &out_layout, AV_SAMPLE_FMT_S16, 16000, &codec_ctx->ch_layout,
                          codec_ctx->sample_fmt, codec_ctx->sample_rate, 0, nullptr);
  if (ret < 0 || !swr_raw || swr_init(swr_raw) < 0)
    throw std::runtime_error("LibAv: Swr setup failed");
  ...
  if (!packet || !frame)
    throw std::runtime_error("LibAv: Allocation failed");
...
ArrayXXbf WhisperPreprocessor::_log_mel_spectrogram(ArrayXf& audio_tensor) {
  ...
  Eigen::Map<ArrayXXcd> stft_result(reinterpret_cast<std::complex<double>*>(_fft_out), N_FRAMES,
                                    N_FFT / 2 + 1);

  // Post process.
  ArrayXXd tmp = ((stft_result.abs2().matrix() * _mel_filters.matrix().transpose())
                      .array()
                      .cwiseMax(1e-10)
                      .log10());
...
WhisperModel::TranscriptionResult
WhisperModel::run_model(const std::filesystem::path& audio_file_name, const std::string& language,
...
WhisperModel::TranscriptionResult WhisperModel::run_model_from_pcm(std::span<const float> pcm,
...
WhisperModel::TranscriptionResult WhisperModel::_run_model(const ArrayXXbf& mel,
                                                           const std::string& language,
                                                           const std::string& task) {
  ...
  if (_is_auto_language(language)) {
    ...
    _logger->info("Detected language: {} token={} index={}", resolved_language,
                  detected_language_token, detected_language_index);
  } else {
  ...
  const uint8_t* token_embeddings_ptr =
      reinterpret_cast<const uint8_t*>(get_buffer("token_embeddings").get_virtual_addr());
  ...
  for (uint32_t i = 0; i < _input_token_ids.size(); ++i) {
    decoder_init_buf.upload(token_embeddings_ptr + _input_token_ids[i] * token_embed_size,
                            i * token_embed_size, token_embed_size,
  ...
  if (new_tokens.back() == _stop_token_id) {
    ...
    return {_tokenizer_ptr->decode(new_tokens, true), resolved_language, resolved_task,
            ...
            _cfg.log_probe_enabled ? std::optional<float>{logprob_sum / logprob_count}
                                   : std::optional<float>{}};
  }

  // Run decoder pre/cache/post models to generate other tokens.
  ChronoTimer timer_tps(true);
  for (uint32_t token_idx = _input_token_ids.size(); token_idx < _cfg.max_target_positions;
       ++token_idx) {
    ...
    for (uint8_t layer_idx = 0; layer_idx < _cfg.decoder_layers; ++layer_idx) {
      ...
      if (layer_idx == 0) {
        ifm_map.emplace(std::piecewise_construct, std::forward_as_tuple(0),
                        std::forward_as_tuple(&get_buffer("token_embeddings"),
  ...
  return {_tokenizer_ptr->decode(new_tokens, true), resolved_language, resolved_task,
          language_detect_result.no_speech_prob, avg_logprob};
}
...
void WhisperModel::_initialize() {
  ...
  _input_token_ids = {_tokenizer_ptr->token_to_id("<|startoftranscript|>"),
  ...
  future_token_mask_buf.upload(decoder_future_token_mask.data(), _cfg.max_target_positions * 2,
                               decoder_future_token_mask.size() * 2, true);
...
void WhisperModel::_define_buffers() {
  ...
  for (uint32_t i = 0; i < _cfg.decoder_layers; ++i) {
    ...
    define_buffer(
        ...
        {_cfg.decoder_attention_heads, _cfg.max_source_positions, _cfg.get_decoder_head_dim()});
    define_buffer(fmt::format("decoder_cache_key{}", i), {_cfg.max_target_positions, _cfg.d_model});
    define_buffer(fmt::format("decoder_cache_val{}", i), {_cfg.max_target_positions, _cfg.d_model});
  }
  ...
  define_buffer("decoder_n1_buffer2",
                {_cfg.decoder_attention_heads, 1, _cfg.get_decoder_head_dim()});
...
void WhisperModel::_define_model(
    ...
    const std::filesystem::path& model_path, const std::vector<MLABufferSlice>& ifms,
  ...
  if (model_type == "encoder") {
    ...
    _decoder_language_detect_model_ptr =
        std::make_unique<MLAModelWithBuffer>(model_path, ifms, ofms);
  } else if (model_type == "decoder_init") {
    _decoder_init_model_map.emplace(std::piecewise_construct,
    ...
    _decoder_init_log_probe_model_map.emplace(std::piecewise_construct,
                                              std::forward_as_tuple(std::get<uint8_t>(key)),
                                              std::forward_as_tuple(model_path, ifms, ofms));
  } else if (model_type == "decoder_pre") {
    _decoder_pre_model_map.emplace(std::piecewise_construct,
    ...
    _decoder_cache_model_map.emplace(
        std::piecewise_construct, std::forward_as_tuple(std::get<WhisperDecoderModelMapKey>(key)),
        std::forward_as_tuple(model_path, ifms, ofms));
  } else if (model_type == "decoder_post") {
    _decoder_post_model_map.emplace(std::piecewise_construct,
                                    std::forward_as_tuple(std::get<WhisperDecoderModelMapKey>(key)),
                                    std::forward_as_tuple(model_path, ifms, ofms));
  } else {
...
void WhisperModel::_define_models() {
  // Encoder model.
  _define_model("encoder", {}, _get_elf_path_encoder(),
                std::vector<MLABufferSlice>{{&get_buffer("encoder_ifm")}},
                std::vector<MLABufferSlice>{{&get_buffer("encoder_ofm")}});

  _define_model("decoder_language_detect", {}, _get_elf_path_decoder_language_detect(),
                std::vector<MLABufferSlice>{{&get_buffer("encoder_ofm")}},
  ...
  for (uint8_t layer_idx = 0; layer_idx < _cfg.decoder_layers; ++layer_idx) {
    ...
    ofms.emplace_back(&get_buffer(fmt::format("decoder_cache_key{}", layer_idx)),
                      std::vector<uint32_t>{0, 0},
                      std::vector<uint32_t>{num_input_tokens, _cfg.d_model});
    ofms.emplace_back(&get_buffer(fmt::format("decoder_cache_val{}", layer_idx)),
                      std::vector<uint32_t>{0, 0},
                      std::vector<uint32_t>{num_input_tokens, _cfg.d_model});
    ofms.emplace_back(&get_buffer(fmt::format("encoder_cache_key{}", layer_idx)),
                      std::vector<uint32_t>{0, 0, 0},
                      std::vector<uint32_t>{_cfg.decoder_attention_heads, _cfg.max_source_positions,
                                            _cfg.get_decoder_head_dim()});
    ofms.emplace_back(&get_buffer(fmt::format("encoder_cache_val{}", layer_idx)),
                      std::vector<uint32_t>{0, 0, 0},
                      std::vector<uint32_t>{_cfg.decoder_attention_heads, _cfg.max_source_positions,
                                            _cfg.get_decoder_head_dim()});
    ...
    if (_cfg.log_probe_enabled && layer_idx == _cfg.decoder_layers - 1) {
      ...
      log_probe_ofms.emplace_back(&get_buffer(fmt::format("decoder_cache_key{}", layer_idx)),
                                  std::vector<uint32_t>{0, 0},
                                  std::vector<uint32_t>{num_input_tokens, _cfg.d_model});
      log_probe_ofms.emplace_back(&get_buffer(fmt::format("decoder_cache_val{}", layer_idx)),
                                  std::vector<uint32_t>{0, 0},
                                  std::vector<uint32_t>{num_input_tokens, _cfg.d_model});
      log_probe_ofms.emplace_back(&get_buffer(fmt::format("encoder_cache_key{}", layer_idx)),
                                  std::vector<uint32_t>{0, 0, 0},
                                  std::vector<uint32_t>{_cfg.decoder_attention_heads,
                                                        _cfg.max_source_positions,
                                                        _cfg.get_decoder_head_dim()});
      log_probe_ofms.emplace_back(&get_buffer(fmt::format("encoder_cache_val{}", layer_idx)),
                                  std::vector<uint32_t>{0, 0, 0},
                                  std::vector<uint32_t>{_cfg.decoder_attention_heads,
                                                        _cfg.max_source_positions,
                                                        _cfg.get_decoder_head_dim()});
      _define_model("decoder_init_log_probe", layer_idx,
                    _get_elf_path_decoder_init_log_probe(layer_idx), ifms, log_probe_ofms);
    }
  }

  // Decoder pre/cache/post.
  for (uint32_t token_idx = num_input_tokens; token_idx < _cfg.max_target_positions; ++token_idx) {
    for (uint8_t layer_idx = 0; layer_idx < _cfg.decoder_layers; ++layer_idx) {
      WhisperDecoderModelMapKey model_key{layer_idx, token_idx};
      ...
      if (layer_idx == 0) {
        ...
        pre_ifms.emplace_back(&get_buffer("position_embeddings"),
      ...
      pre_ofms.emplace_back(&get_buffer(fmt::format("decoder_cache_key{}", layer_idx)),
                            std::vector<uint32_t>{token_idx, 0},
                            std::vector<uint32_t>{1, _cfg.d_model});
      pre_ofms.emplace_back(&get_buffer(fmt::format("decoder_cache_val{}", layer_idx)),
                            std::vector<uint32_t>{token_idx, 0},
                            std::vector<uint32_t>{1, _cfg.d_model});
      _define_model("decoder_pre", model_key, _get_elf_path_decoder_pre(layer_idx), pre_ifms,
                    pre_ofms);
      ...
      if (_cfg.decoder_use_future_token_mask) {
        std::vector<MLABufferSlice> cache_ifms{
            ...
            {&get_buffer(fmt::format("decoder_cache_val{}", layer_idx)),
             {0, 0},
             {_cfg.max_target_positions, _cfg.d_model}}};
        _define_model("decoder_cache", model_key,
                      _get_elf_path_decoder_cache(_cfg.max_target_positions - 1), cache_ifms,
                      cache_ofms);
      } else {
        std::vector<MLABufferSlice> cache_ifms{
            pre_ofms[0],
            ...
            {&get_buffer(fmt::format("decoder_cache_val{}", layer_idx)),
             {0, 0},
             {token_idx + 1, _cfg.d_model}}};
        _define_model("decoder_cache", model_key, _get_elf_path_decoder_cache(token_idx),
                      cache_ifms, cache_ofms);
      }
      ...
      _define_model("decoder_post", model_key, _get_elf_path_decoder_post(layer_idx), post_ifms,
                    post_ofms);
    }
  }
}
...
std::filesystem::path WhisperModel::_get_elf_path_decoder_init(uint8_t layer_idx) const {
  auto elf_file_name =
      fmt::format("{}_decoder_init_layer{}_stage1_mla.elf", _cfg.model_name, layer_idx);
...
std::filesystem::path WhisperModel::_get_elf_path_decoder_init_log_probe(uint8_t layer_idx) const {
  auto elf_file_name =
      fmt::format("{}_decoder_init_log_probe_layer{}_stage1_mla.elf", _cfg.model_name, layer_idx);
  return _elf_dir / elf_file_name;
}

std::filesystem::path WhisperModel::_get_elf_path_decoder_pre(uint8_t layer_idx) const {
  auto elf_file_name =
      fmt::format("{}_decoder_n1_pre_layer{}_stage1_mla.elf", _cfg.model_name, layer_idx);
  return _elf_dir / elf_file_name;
}

std::filesystem::path WhisperModel::_get_elf_path_decoder_cache(uint16_t token_idx) const {
  auto elf_file_name =
      fmt::format("{}_decoder_n1_cache_token{}_stage1_mla.elf", _cfg.model_name, token_idx);
  return _elf_dir / elf_file_name;
}

std::filesystem::path WhisperModel::_get_elf_path_decoder_post(uint8_t layer_idx) const {
  auto elf_file_name =
      fmt::format("{}_decoder_n1_post_layer{}_stage1_mla.elf", _cfg.model_name, layer_idx);
  return _elf_dir / elf_file_name;
}

std::filesystem::path WhisperModel::_get_elf_path_decoder_language_detect() const {
  return _elf_dir / fmt::format("{}_decoder_language_detect_stage1_mla.elf", _cfg.model_name);
}
...
WhisperModel::LanguageDetectResult WhisperModel::_run_language_detect() {
  ...
  logits_buf.invalidate_cache();
  return {token_ptr[0], _compute_token_prob(logits_buf, no_speech_token_id)};
}
...
uint32_t WhisperModel::_language_token_from_index(uint32_t language_idx) const {
  if (language_idx >= _cfg.language_token_ids.size()) {
    throw std::runtime_error(fmt::format("Invalid detected language index: {}", language_idx));
  }
...
std::string WhisperModel::_language_code_from_token(uint32_t token_id) const {
  auto it = std::find(_cfg.language_token_ids.begin(), _cfg.language_token_ids.end(), token_id);
...
std::string WhisperModel::_update_language(const std::string& language) {
  ...
  if (auto it = std::find(_cfg.language_codes.begin(), _cfg.language_codes.end(), language_code);
