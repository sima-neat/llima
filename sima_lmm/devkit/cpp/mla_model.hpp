...
public:
  MLAModelWithBuffer(std::filesystem::path model_path, std::vector<MLABufferSlice> ifms,
                     std::vector<MLABufferSlice> ofms);
  ~MLAModelWithBuffer(){};

  void load();
  void free();
  void run(std::map<uint8_t, MLABufferSlice>* ifm_map_ptr = nullptr,
           std::map<uint8_t, MLABufferSlice>* ofm_map_ptr = nullptr);
  void add_to_queue(std::map<uint8_t, MLABufferSlice>* ifm_map_ptr = nullptr,
                    std::map<uint8_t, MLABufferSlice>* ofm_map_ptr = nullptr);
  void update_reloc(const std::map<std::string, uint64_t>& reloc_addr_map);

  static void initialize();
  static void run_queue();
  static void load_all_models(std::optional<std::filesystem::path> relative_dir = std::nullopt);
  static void free_all_models(std::optional<std::filesystem::path> relative_dir = std::nullopt);
  ...
  static void read_env_vars() {
    ...
    _disable_parallel_load =
        get_env_var("SIMA_LLIMA_RUN_DISABLE_PARALLEL_LOAD", _disable_parallel_load);
  }

private:
  void _debug_inouts(const std::string& name, std::map<uint8_t, MLABufferSlice>* fm_map_ptr);
  simaaidispatcher::PreparedMlaRunRef
  _prepare_run_ref(std::map<uint8_t, MLABufferSlice>* ifm_map_ptr,
