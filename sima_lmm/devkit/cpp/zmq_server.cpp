...
ZMQServer::ZMQServer(const std::filesystem::path& model_path, uint32_t port)
...
void ZMQServer::run() {
  ...
  try {
    while (_is_running) {
      ...
      if (request_metadata.tensor_shape.size() != 2 || request_metadata.tensor_shape[0] != 1 ||
          request_metadata.tensor_shape[1] <= 0)
        throw std::runtime_error(fmt::format("Invalid shape: {}", request_metadata.tensor_shape));
      if (request_metadata.tensor_dtype != "uint32")
        throw std::runtime_error(fmt::format("Invalid dtype: {}", request_metadata.tensor_dtype));
      _logger->info("type: {}, dtype: {}, shape: {}, max_num_tokens: {}, stop_token_ids: {}",
                    request_metadata.type, request_metadata.tensor_dtype,
                    request_metadata.tensor_shape, request_metadata.max_num_tokens.value_or(0),
      ...
      if (request_metadata.type == "generate") {
        ...
        auto result = _vision_language_model_ptr->run_model(
            input_token_ids, request_metadata.max_num_tokens, request_metadata.stop_token_ids);
        ...
        response_metadata.tensor_shape = {1, input_token_ids.size(),
                                          result.size() / input_token_ids.size()};
        ...
        throw std::runtime_error(fmt::format("Unknown request type: {}", request_metadata.type));
      }
      ...
      response_messages[0] = {response_metadata_sbuffer.data(), response_metadata_sbuffer.size()};
