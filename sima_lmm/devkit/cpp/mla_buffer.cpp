...
MLABuffer::MLABuffer(std::string name, std::vector<size_t> shape, std::string dtype,
                     bool align_last_dim)
    : _name(std::move(name)), _shape(std::move(shape)), _dtype(std::move(dtype)),
      _align_last_dim(align_last_dim), _simaai_mem_ptr(nullptr) {

  if (!_shape.size())
    throw std::runtime_error(std::string("Shape needs to have at least 1 dimension") +
...
void MLABuffer::allocate() {
  ...
  // Allocate the buffer.
  _simaai_mem_ptr =
      simaai_memory_alloc_flags(_size_padded, SIMAAI_MEM_TARGET_DMS0, SIMAAI_MEM_FLAG_CACHED);
  if (!_simaai_mem_ptr) {
    throw std::runtime_error("Failed to allocate buffer (" + _name + "): " + std::strerror(errno));
  }
...
void MLABuffer::load_stream(std::istream& stream) {
  ...
  if (_align_last_dim && row_size != padded_row_size) {
    ...
    for (size_t i = 0; i < num_rows; ++i) {
      stream.read(reinterpret_cast<char*>(_virtual_addr) + i * padded_row_size, row_size);
    }
  } else {
...
void MLABuffer::upload(const void* data, size_t data_begin, size_t data_size, bool flush) {
  if (_align_last_dim && (_shape.back() % MLA_ROW_SIZE != 0)) {
    ...
    for (uint32_t i = 0; i < num_last_dims; ++i) {
      std::memcpy(reinterpret_cast<uint8_t*>(_virtual_addr) + i * last_dim_padded,
                  reinterpret_cast<const uint8_t*>(data) + i * last_dim, last_dim);
    }
...
void MLABuffer::download(void* data) const {
  ...
  if (_align_last_dim && (_shape.back() % MLA_ROW_SIZE != 0)) {
    ...
    for (uint32_t i = 0; i < num_last_dims; ++i) {
      std::memcpy(reinterpret_cast<uint8_t*>(data) + i * last_dim,
                  reinterpret_cast<const uint8_t*>(_virtual_addr) + i * last_dim_padded, last_dim);
    }
  } else {
...
void MLABuffer::print(std::ostream& s, const std::optional<std::vector<uint32_t>>& override_begin,
                      const std::optional<std::vector<uint32_t>>& override_shape) const {
  // Basic info.
  ...
  std::vector<uint32_t> shape =
      override_shape.value_or(std::vector<uint32_t>(_shape.begin(), _shape.end()));
  ...
  if (_dtype == "int8")
    ...
    print_content<Eigen::bfloat16>(s, 0, reinterpret_cast<Eigen::bfloat16*>(_virtual_addr), begin,
                                   shape);
...
template <typename T>
void MLABuffer::print_content(std::ostream& s, uint32_t level, const T* curr_ptr,
...
MLABufferSlice::MLABufferSlice(MLABuffer* buf_ptr)
    : _buf_ptr(buf_ptr), _begins(std::nullopt), _shapes(std::nullopt) {}

MLABufferSlice::MLABufferSlice(MLABuffer* buf_ptr, std::vector<uint32_t> begins)
