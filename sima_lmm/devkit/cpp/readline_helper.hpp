...
//||   Unpublished Copyright (c) 2026 SiMa.ai, All Rights Reserved.  ||
//**************************************************************************
...
public:
    ...
    explicit ReadlineSupport (size_t max_entries = DEFAULT_MAX_ENTRIES)
    ...
    void add_to_history(const std::string& line) {
        ...
        if (prev && prev->line && line == prev->line) return;
...
private:
    ...
    static inline std::filesystem::path history_path() {
        ...
        if (!home || !*home) home = "."; // Chose the current-dir
    ...
    char** generate_completions(const char* text, int start) const {
        ...
        if (!candidates || candidates->empty()) return nullptr;
