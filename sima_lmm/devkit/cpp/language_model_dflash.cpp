...
const std::vector<uint32_t> &LanguageModel::_run_dflash_draft(
  ...
  for (uint8_t layer_idx = 0; layer_idx < _cfg.lm_cfg.num_hidden_layers;
    ...
    attn_models.pre->add_to_queue(&pre_inputs);
    attn_models.cache->add_to_queue();
    attn_models.post->add_to_queue(layer_idx == 0 ? &pre_inputs : nullptr);
  }
  MLAModelWithBuffer::run_queue();
  ...
  auto add_view = [&](MLABuffer &buffer, uint32_t begin, uint32_t width) {
    ...
    views.push_back(DFlashLogitView{
        &buffer, static_cast<const uint8_t *>(buffer.get_virtual_addr()),
...
#pragma omp parallel for schedule(static)
  for (int row = 0; row < static_cast<int>(proposals.size()); ++row) {
    ...
    for (const auto &view : views) {
      ...
      const auto [split_best, split_index] =
          argmax_bf16(logits, view.width);
      const uint32_t token = view.vocab_begin + split_index;
      if (split_best > best || (split_best == best && token < best_token)) {
        best = split_best;
        best_token = token;
      }
    }
...
LanguageModel::_run_dflash_target_verify(std::span<const uint32_t> input_ids,
  ...
  for (uint8_t layer_idx = 0; layer_idx < _cfg.lm_cfg.num_hidden_layers;
    ...
    if (is_attention(layer_type)) {
      ...
      const auto attn_models =
          _bind_attn_models(num_tokens, token_idx, layer_idx, input_scale);
      attn_models.pre->add_to_queue(input_overrides);
      attn_models.cache->add_to_queue();
      attn_models.post->add_to_queue(input_overrides, output_overrides);
  ...
  for (uint16_t row = 0; row < num_tokens; ++row) {
    ...
    for (const auto &view : views) {
      ...
      const uint32_t token = view.vocab_begin + split_index;
      if (split_best > best
          || (split_best == best && token < target_token)) {
        best = split_best;
        target_token = token;
      }
    }
