...
VisionModel::VisionModel(std::filesystem::path model_path)
    : BaseModel(model_path), _vm_cfg(_cfg.vm_cfg.value()), _mm_cfg(_cfg.mm_cfg.value()) {
  _initialize();
}

std::vector<Eigen::bfloat16>
VisionModel::run_model(const std::vector<Eigen::bfloat16>& ifm_tensor) {
  // Upload the ifm.
...
void VisionModel::run_model(const std::vector<Eigen::bfloat16>& ifm_tensor,
...
void VisionModel::_define_buffers() {
  if (_cfg.model_type.starts_with("vlm-lfm2") || _cfg.model_type.starts_with("vlm-qwen") ||
    ...
    uint32_t patch_feature_size = (_vm_cfg.temporal_patch_size * _vm_cfg.spatial_patch_size *
                                   _vm_cfg.spatial_patch_size * num_channels);
    uint32_t seq_len =
        (_vm_cfg.num_spatial_patches[0] * _vm_cfg.num_spatial_patches[1] + _vm_cfg.cls_embed);
    define_buffer("vision_ifm", {seq_len, patch_feature_size}, "bfloat16", false);
  } else {
    define_buffer("vision_ifm", {_vm_cfg.image_sizes[0], _vm_cfg.image_sizes[1], 3}, "bfloat16",
                  false);
  }
  ...
  for (size_t i = 0; i < _vm_cfg.deepstack_visual_indexes.size(); ++i) {
    define_buffer(fmt::format("deepstack_feature_l{}", i),
...
void VisionModel::_define_models() {
  ...
  _model_ptr = std::make_unique<MLAModelWithBuffer>(
      ...
      std::vector<MLABufferSlice>{MLABufferSlice{&get_buffer("vision_ifm")}}, std::move(ofms));
}
