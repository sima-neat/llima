...
public:
  VisionLanguageModel(std::filesystem::path model_path,
                      std::optional<std::string> system_prompt = std::nullopt,
                      std::optional<std::string> chat_template = std::nullopt);
  ~VisionLanguageModel() {}

  std::optional<std::string> run_model(const Chat& chat,
  ...
  std::vector<Eigen::bfloat16> run_model_for_logits(std::span<const uint32_t> input_token_ids);
  std::vector<double>
  run_model_for_ttnt(std::span<const uint32_t> input_token_ids,
                     std::optional<uint16_t> override_max_num_tokens = std::nullopt,
                     std::optional<std::set<uint32_t>> override_stop_token_ids = std::nullopt);
  void stop_model();
  ...
  void set_draft_vlm(VisionLanguageModel* draft_vlm) {
    _draft_vlm_ptr = draft_vlm;
  }

  Chat create_chat() {
    return Chat(_vlm_helper);
  }
  bool support_image() const {
    return _cfg.support_image();
  }
  ...
  void unset_reloc() {
    _language_model_ptr->unset_reloc();
  }
