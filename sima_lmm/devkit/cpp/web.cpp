...
WEB::WEB(std::filesystem::path vlm_model_path,
...
void WEB::run() {
  _http_server.Post("/stop", [this](const httplib::Request& req, httplib::Response& res) {
    this->_handle_stop(req, res);
  });

  _http_server.Post("/set_lora", [this](const httplib::Request& req, httplib::Response& res) {
    this->_handle_set_lora(req, res);
  });

  _http_server.Post("/unset_lora", [this](const httplib::Request& req, httplib::Response& res) {
  ...
  for (const auto& msg : messages) {
    ...
    if (internal_content.is_array() && !internal_content.empty()) {
      ...
      const bool has_valid_tool_calls =
          msg.contains("tool_calls") && msg["tool_calls"].is_array() && !msg["tool_calls"].empty();
...
std::string WEB::_format_openai_sse_chunk(const std::string& content, const std::string& model,
                                          bool finished, std::optional<std::string> finish_reason,
                                          std::optional<double> ttft, std::optional<double> tps,
                                          bool from_draft) {
  nlohmann::json chunk;
...
std::string WEB::_format_ollama_ndjson_chunk(const std::string& content, const std::string& model,
                                             bool finished,
...
std::string WEB::_format_audio_sse_chunk(const std::string& text, const std::string& event_task,
                                         bool finished, std::optional<std::string> finish_reason,
                                         std::optional<double> ttft, std::optional<double> tps,
  ...
  nlohmann::json chunk;
  const auto object_prefix =
      event_task == "translate" ? "audio.translation" : "audio.transcription";
...
void WEB::_handle_chat_completions(const httplib::Request& req, httplib::Response& res,
                                   bool is_openai) {
  _set_cors_headers(res);
  try {
    std::string model;
    ...
    if (is_openai) {
      error_body = {{"error", {{"message", e.what()}, {"type", "internal_error"}}}};
    }
...
void WEB::_handle_audio_transcriptions(const httplib::Request& req, httplib::Response& res,
  ...
  try {
    ...
    nlohmann::json response = {{"text", result.text},
...
void WEB::_execute_streaming_audio_transcription(httplib::Response& res,
  ...
  res.set_chunked_content_provider(
      "text/event-stream", [this, language, task](size_t offset, httplib::DataSink& sink) {
        (void)offset;
        ...
        try {
          ...
          auto result = _whisper_model_ptr->run_model(_AUDIO_FILE_NAME, language, task);
          std::string chunk =
              _format_audio_sse_chunk("", task, true, finish_reason.value_or("stop"), ttft_value,
                                      tps_value, result.language, result.task,
          ...
          nlohmann::json error;
          error["object"] =
              task == "translate" ? "audio.translation.error" : "audio.transcription.error";
          error["error"] = e.what();
...
std::optional<Chat> WEB::_prepare_chat_context(const httplib::Request& req, httplib::Response& res,
...
void WEB::_execute_streaming_chat(httplib::Response& res, Chat& chat, const std::string& model,
                                  bool is_openai) {
  ...
  res.set_chunked_content_provider(
      ...
      [this, chat, model, is_openai](size_t offset, httplib::DataSink& sink) {
        ...
        auto info_callback = [&](const std::string& metric_type, double metric_value) {
          ...
          if (metric_type == "END" || metric_type == "FULL") {
            const std::string default_finish_reason = (metric_type == "FULL") ? "length" : "stop";
              ...
              if (is_openai) {
                ...
                if (!parsed_tool_calls.is_null()) {
                  ...
                  std::string content_chunk = _format_openai_sse_chunk(
                      buffered_text, model, false, std::nullopt, ttft_for_chunk, tps_value);
                ...
                std::string final_chunk =
                    _format_openai_sse_chunk("", model, true, finish_reason) + "data: [DONE]\n\n";
                ...
                if (!parsed_tool_calls.is_null()) {
                  message["content"] = "";
                  message["tool_calls"] = openai_tool_calls_to_ollama(parsed_tool_calls);
                } else {
            ...
            if (is_openai) {
              ...
              formatted_chunk = _format_ollama_ndjson_chunk("", model, true, finish_reason);
            }
        ...
        auto text_callback = [&](const std::string& text, bool stream_end, bool from_draft) {
          ...
          if (is_openai && !sent_initial_chunk) {
            ...
            initial_choice["delta"] = {{"role", "assistant"}, {"content", nullptr}};
          ...
          if (is_openai) {
            ...
            formatted_chunk = _format_openai_sse_chunk(text, model, false, std::nullopt,
                                                       ttft_for_chunk, tps_value, from_draft);
          } else {
            std::optional<double> ttft_for_chunk = std::nullopt;
            if (!ttft_sent && ttft_value.has_value()) {
              ttft_for_chunk = ttft_value;
            ...
            formatted_chunk = _format_ollama_ndjson_chunk(text, model, false, std::nullopt,
                                                          ttft_for_chunk, tps_value, from_draft);
          }
        ...
        _vlm_thread = std::jthread([this, chat]() { _vision_language_model_ptr->run_model(chat); });
...
static nlohmann::json build_tool_call_entry(const nlohmann::json& parsed, int& id_counter) {
  ...
  std::string id =
      parsed.contains("id") && parsed["id"].is_string()
...
//   Mistral-style:  [{"name":...}]      ([TOOL_CALLS] special token stripped, leaving bare JSON
//   array) Qwen-style:     <tool_call>\n{"name": "...", "arguments": {...}}\n</tool_call>
  ...
  try {
    if (text.starts_with("call:")) {
      ...
      while (pos < text.size() && text.substr(pos).starts_with("call:")) {
        ...
        if (close == std::string_view::npos)
          return nullptr;
        std::string args = gemma4_bare_to_json(std::string(text.substr(brace, close - brace + 1)));
        auto entry =
...
void WEB::_execute_normal_chat(httplib::Response& res, Chat& chat, const std::string& model,
                               bool is_openai) {
