...
MlaRunCompletion make_submit_error(simaaidispatcher::DispatcherBase* dispatcher, int32_t rc) {
...
MlaRunCompletion wait_for_result(simaaidispatcher::DispatcherBase* dispatcher,
...
MlaRunCompletion submit_prepared_and_wait(simaaidispatcher::DispatcherBase* dispatcher,
                                          simaaidispatcher::JobMLA&& job) {
  auto promise = std::make_shared<std::promise<MlaRunCompletion>>();
  auto future = promise->get_future();

  job.userData = promise;
  job.cb = [](const std::map<std::string, simaai_memory_t*>&, int32_t rc, std::any user_data) {
    auto completion = std::any_cast<MlaRunPromise>(user_data);
    MlaRunCompletion result;
    result.rc = rc;
...
MlaRunCompletion submit_queue_and_wait(
  ...
  request.cb = [](int32_t rc, std::size_t failed_index,
                  const simaaidispatcher::DispatcherBase::ErrorSnapshot&,
                  const simaaidispatcher::DispatcherBase::ProfileSnapshot&, std::any user_data) {
    auto completion = std::any_cast<MlaRunPromise>(user_data);
    MlaRunCompletion result;
    result.rc = rc;
...
void connect_mla_rt(const std::vector<std::string>& args) {
  ...
  MLAModelWithBuffer::_dispatcher = simaaidispatcher::DispatcherFactory::getDispatcher(
...
MLAModelWithBuffer::MLAModelWithBuffer(std::filesystem::path model_path,
...
void MLAModelWithBuffer::load() {
  ...
  if (!_unique_model_ptrs[_model_idx]) {
    throw std::runtime_error(fmt::format("Failed to load model through MLASHM dispatcher: {} ({})",
                                         model_path, dispatcher->lastErrorString()));
  }
...
void MLAModelWithBuffer::free() {
  ...
  if (rc != 0) {
    spdlog::error("Failed to release model through MLASHM dispatcher: {} ({})",
                  _unique_model_paths[_model_idx], dispatcher->lastErrorString());
  }
...
simaaidispatcher::PreparedMlaRunRef
MLAModelWithBuffer::_prepare_run_ref(std::map<uint8_t, MLABufferSlice>* ifm_map_ptr,
                                     std::map<uint8_t, MLABufferSlice>* ofm_map_ptr) {
  auto patch_io = [](const std::vector<MLABufferSlice>& default_slices,
                     std::map<uint8_t, MLABufferSlice>* override_map_ptr,
                     std::vector<uint64_t>& paddr, std::vector<int>& len) {
    for (uint32_t i = 0; i < default_slices.size(); ++i) {
      ...
      MLABuffer* base = effective_slice->get_buf_ptr() ? effective_slice->get_buf_ptr()
...
void MLAModelWithBuffer::run(std::map<uint8_t, MLABufferSlice>* ifm_map_ptr,
                             std::map<uint8_t, MLABufferSlice>* ofm_map_ptr) {
  load();
  ...
  if (result.rc != 0) {
    throw std::runtime_error(fmt::format(
        "MLASHM dispatcher run failed for {}: rc={} ({})", _unique_model_paths[_model_idx],
        result.rc, result.detail.empty() ? dispatcher->lastErrorString() : result.detail));
  }

  _debug_inouts("ofm", ofm_map_ptr);
}

void MLAModelWithBuffer::add_to_queue(std::map<uint8_t, MLABufferSlice>* ifm_map_ptr,
                                      std::map<uint8_t, MLABufferSlice>* ofm_map_ptr) {
...
void MLAModelWithBuffer::run_queue() {
  if (!MLAModelWithBuffer::_enable_queue || MLAModelWithBuffer::_queue_request.runs.empty())
    return;

  auto* dispatcher = _get_dispatcher();
  try {
    auto result = submit_queue_and_wait(dispatcher, std::move(MLAModelWithBuffer::_queue_request));
    if (result.rc != 0) {
      throw std::runtime_error(
          fmt::format("MLASHM dispatcher runQueue failed: rc={} failed_index={} ({})", result.rc,
                      result.failed_index,
...
void MLAModelWithBuffer::update_reloc(const std::map<std::string, uint64_t>& reloc_addr_map) {
  ...
  if (rc != 0) {
    throw std::runtime_error(
        fmt::format("Failed to update relocations through MLASHM dispatcher: {} rc={} ({})",
                    _unique_model_paths[_model_idx], rc, dispatcher->lastErrorString()));
  }
}
...
void MLAModelWithBuffer::load_all_models(std::optional<std::filesystem::path> relative_dir) {
  ...
  for (const auto& [file_name, idx] : _unique_model_path_to_idx_map) {
    if (relative_dir.has_value() &&
        !file_name.string().starts_with(relative_dir.value().string())) {
      continue;
    }
  ...
  if (!_disable_parallel_load) {
    ...
    if (handles.size() != file_names.size()) {
      throw std::runtime_error(
          fmt::format("Bulk MLASHM model load returned {} handles for {} models ({})",
                      handles.size(), file_names.size(), dispatcher->lastErrorString()));
    }
    for (std::size_t i = 0; i < file_names.size(); ++i) {
      _unique_model_ptrs[indices[i]] = handles[i];
      if (!_unique_model_ptrs[indices[i]]) {
        throw std::runtime_error(
            fmt::format("Failed to bulk load model through MLASHM dispatcher: {} ({})",
                        file_names[i], dispatcher->lastErrorString()));
      }
  ...
  for (std::size_t i = 0; i < file_names.size(); ++i) {
    ...
    if (!_unique_model_ptrs[indices[i]]) {
      throw std::runtime_error(
          fmt::format("Failed to load model through MLASHM dispatcher: {} ({})", model_path,
...
void MLAModelWithBuffer::free_all_models(std::optional<std::filesystem::path> relative_dir) {
  ...
  for (std::size_t i = 0; i < MLAModelWithBuffer::_unique_model_paths.size(); ++i) {
    ...
    if (rc != 0) {
      spdlog::error("Failed to release model through MLASHM dispatcher: {} ({})",
                    MLAModelWithBuffer::_unique_model_paths[i], dispatcher->lastErrorString());
    }
    MLAModelWithBuffer::_unique_model_ptrs[i] = nullptr;
  }
}

void MLAModelWithBuffer::_debug_inouts(const std::string& name,
  ...
  if (_print_inouts) {
    ...
    for (uint32_t i = 0; i < fms.size(); ++i) {
      ...
      if (fm_map_ptr && fm_map_ptr->contains(i)) {
        ...
        auto buf_ptr = (buf_slice.get_buf_ptr() ? buf_slice.get_buf_ptr() : fms[i].get_buf_ptr());
  ...
  if (_save_inouts) {
    ...
    for (uint32_t i = 0; i < fms.size(); ++i) {
      std::filesystem::path d =
          (_save_inout_dir / _unique_model_paths[_model_idx].stem() / (name + std::to_string(i)));
