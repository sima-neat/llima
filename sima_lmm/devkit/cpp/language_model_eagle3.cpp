...
LanguageModel::DraftForwardResult LanguageModel::run_eagle3_draft_model(
    LanguageModel& target_lm, std::vector<Eigen::bfloat16> hidden_states,
    std::vector<uint32_t> input_ids, std::optional<std::vector<uint8_t>> attention_mask,
    std::optional<std::vector<int32_t>> position_ids, int num_cached_tokens, bool is_prefill) {
  ...
  if (position_ids.has_value()) {
    ...
    std::vector<Eigen::bfloat16> freq_real_padded(static_cast<size_t>(max_num_tokens) * freq_dim,
                                                  Eigen::bfloat16{0.0f});
    std::vector<Eigen::bfloat16> freq_imag_padded(static_cast<size_t>(max_num_tokens) * freq_dim,
                                                  Eigen::bfloat16{0.0f});
  ...
  auto mask = eagle_helpers::prepare_attention_mask(attention_mask, seq_length,
                                                    past_key_values_length, _eagle3_tree_mask);
  ...
  const auto* embeddings_ptr =
      reinterpret_cast<const Eigen::bfloat16*>(target_embeddings_buf.get_virtual_addr());
  ...
  for (size_t i = 0; i < seq_length; ++i) {
    ...
    std::memcpy(input_embeds.data() + i * embed_size, embeddings_ptr + token_id * embed_size,
                row_bytes);
  }
  ...
  if (hidden_cols != embed_size) {
    ...
    if (it == _fc_model_map.end()) {
      throw std::runtime_error("run_eagle3_draft_model: FC model not defined for n=" +
    ...
    ifm_map.emplace(std::piecewise_construct, std::forward_as_tuple(0),
                    std::forward_as_tuple(&fc_input, std::vector<uint32_t>{0, 0},
    ...
    std::vector<Eigen::bfloat16> hidden_states_padded(static_cast<size_t>(num_tokens) * embed_size,
                                                      Eigen::bfloat16{0.0f});
    std::memcpy(hidden_states_padded.data(), hidden_states.data(),
  ...
  std::vector<Eigen::bfloat16> input_embeds_padded(static_cast<size_t>(num_tokens) * embed_size,
                                                   Eigen::bfloat16{0.0f});
  std::memcpy(input_embeds_padded.data(), input_embeds.data(),
              seq_length * embed_size * sizeof(Eigen::bfloat16));
  ...
  pre_ifm_map.emplace(std::piecewise_construct, std::forward_as_tuple(0),
                      std::forward_as_tuple(&token_embeds_buf, std::vector<uint32_t>{0, 0},
                                            std::vector<uint32_t>{num_tokens, embed_size}));
  pre_ifm_map.emplace(std::piecewise_construct, std::forward_as_tuple(1),
                      std::forward_as_tuple(&fc_output_buf, std::vector<uint32_t>{0, 0},
                                            std::vector<uint32_t>{num_tokens, embed_size}));
  // Override freq IFMs (2, 3) to read from row 0 — both the sliced and the
  // pristine linear paths upload starting at row 0.
  {
    const uint32_t freq_dim = freq_real_buf.get_shape().back();
    pre_ifm_map.emplace(std::piecewise_construct, std::forward_as_tuple(2),
                        std::forward_as_tuple(&freq_real_buf, std::vector<uint32_t>{0, 0},
                                              std::vector<uint32_t>{num_tokens, freq_dim}));
    pre_ifm_map.emplace(std::piecewise_construct, std::forward_as_tuple(3),
                        std::forward_as_tuple(&freq_imag_buf, std::vector<uint32_t>{0, 0},
                                              std::vector<uint32_t>{num_tokens, freq_dim}));
  }
...
LanguageModel::TargetVerifyResult
LanguageModel::run_eagle3_target_verify(std::vector<uint32_t> input_ids,
  ...
  if (position_ids.size() != seq_length) {
    throw std::runtime_error(
        fmt::format("run_eagle3_target_verify: position_ids size {} != input_ids size {}",
  ...
  // rotary base.
  auto upload_freq_rows = [&](const RopeTable& host, const std::string& real_name,
    ...
    const uint16_t max_num_tokens = _cfg.pipeline_cfg.max_num_tokens;
    std::vector<Eigen::bfloat16> real_padded(static_cast<size_t>(max_num_tokens) * freq_dim,
                                             Eigen::bfloat16{0.0f});
    std::vector<Eigen::bfloat16> imag_padded(static_cast<size_t>(max_num_tokens) * freq_dim,
                                             Eigen::bfloat16{0.0f});
    for (size_t i = 0; i < seq_length; ++i) {
  ...
  auto attention_mask =
      std::optional<std::vector<uint8_t>>{std::vector<uint8_t>(seq_len_with_past, 1)};
  auto mask = eagle_helpers::prepare_attention_mask(attention_mask, seq_length,
                                                    past_key_values_length, _eagle3_tree_mask);
  ...
  const auto* embeddings_ptr =
      reinterpret_cast<const Eigen::bfloat16*>(embeddings_buf.get_virtual_addr());
  std::vector<Eigen::bfloat16> input_embeds_padded(static_cast<size_t>(num_tokens) * hidden_size,
                                                   Eigen::bfloat16{0.0f});
  const size_t row_bytes = hidden_size * sizeof(Eigen::bfloat16);
  for (size_t i = 0; i < seq_length; ++i) {
    const uint32_t token_id = input_ids[i];
    std::memcpy(input_embeds_padded.data() + i * hidden_size,
                embeddings_ptr + token_id * hidden_size, row_bytes);
  }
  ...
  for (uint8_t layer_idx = 0; layer_idx < num_layers; ++layer_idx) {
    ...
    if (layer_idx == 0) {
      ifm_map.emplace(std::piecewise_construct, std::forward_as_tuple(0),
                      std::forward_as_tuple(&input_embeds_buf, std::vector<uint32_t>{0, 0},
    ...
    auto& layer_freq_real = is_sliding ? get_buffer("local_freq_real") : freq_real_buf;
    auto& layer_freq_imag = is_sliding ? get_buffer("local_freq_imag") : freq_imag_buf;
    const uint32_t layer_freq_dim = layer_freq_real.get_shape().back();
    auto pre_ifm_map = ifm_map;
    pre_ifm_map.emplace(std::piecewise_construct, std::forward_as_tuple(1),
                        std::forward_as_tuple(&layer_freq_real, std::vector<uint32_t>{0, 0},
                                              std::vector<uint32_t>{num_tokens, layer_freq_dim}));
    pre_ifm_map.emplace(std::piecewise_construct, std::forward_as_tuple(2),
                        std::forward_as_tuple(&layer_freq_imag, std::vector<uint32_t>{0, 0},
                                              std::vector<uint32_t>{num_tokens, layer_freq_dim}));
    ...
    if (it != capture_layers.end()) {
      ...
      result.hidden_states[cap_idx].assign(full_cap.begin(),
                                           full_cap.begin() + seq_length * hidden_size);
    }
  }
  ...
  for (uint32_t s = 0; s < num_splits; ++s) {
    const std::string buf_name = (num_splits == 1) ? fmt::format("n{}_buffer4", num_tokens)
                                                   : fmt::format("n{}_lm_split{}", num_tokens, s);
    auto& buf = get_buffer(buf_name);
    buf.invalidate_cache();
    const uint32_t this_split_dim =
        (num_splits == 1) ? lm_head_output_size
                          : std::min<uint32_t>(split_dim, lm_head_output_size - s * split_dim);
    std::vector<Eigen::bfloat16> split_data(static_cast<size_t>(num_tokens) * this_split_dim);
    buf.download(split_data.data());
    for (uint16_t r = 0; r < num_tokens; ++r) {
      std::memcpy(result.logits.data() + r * lm_head_output_size + s * split_dim,
                  split_data.data() + r * this_split_dim, this_split_dim * sizeof(Eigen::bfloat16));
    }
  }
...
LanguageModel::TreeDecodingResult
LanguageModel::tree_decoding(std::vector<uint32_t> tree_candidates,
  ...
  auto target_out = run_eagle3_target_verify(std::move(tree_candidates), std::move(position_ids));
  ...
  for (size_t p = 0; p < n_paths; ++p) {
    for (size_t d = 0; d < max_depth; ++d) {
      ...
      if (idx < 0)
        idx = static_cast<int32_t>(K) - 1;
      std::memcpy(result.logits.data() + (p * max_depth + d) * vocab_size,
  ...
  const size_t valid_K =
      target_out.hidden_states.empty() ? 0 : target_out.hidden_states[0].size() / hidden_size;
  ...
  for (size_t i = 0; i < valid_K; ++i) {
    for (size_t cap = 0; cap < target_out.hidden_states.size(); ++cap) {
      std::memcpy(result.hidden_states.data() + (i * 3 + cap) * hidden_size,
...
LanguageModel::UpdateInferenceInputsResult LanguageModel::update_inference_inputs(
    LanguageModel& draft_lm, std::vector<uint32_t> input_ids,
    const std::vector<std::vector<int32_t>>& candidates, size_t best_candidate,
    int32_t accept_length, const std::vector<std::vector<int32_t>>& retrieve_indices,
    int32_t new_token, const std::vector<Eigen::bfloat16>& hidden_state_new,
  ...
  for (size_t i = 0; i < take; ++i) {
    select_indices[i] = static_cast<uint16_t>(retrieve_indices[best_candidate][i] +
                                              static_cast<int32_t>(prev_input_len));
  }

  // Extend input_ids with the accepted path.
  input_ids.reserve(prev_input_len + take + 1);
  for (size_t i = 0; i < take; ++i) {
    input_ids.push_back(static_cast<uint32_t>(candidates[best_candidate][i]));
  }
  ...
  std::vector<Eigen::bfloat16> accept_hidden_state_new(take * 3 * hidden_size);
  for (size_t i = 0; i < take; ++i) {
    const int32_t src_row = retrieve_indices[best_candidate][i];
    std::memcpy(accept_hidden_state_new.data() + i * 3 * hidden_size,
  ...
  const uint32_t bonus_token = static_cast<uint32_t>(std::distance(sample_p.begin(), bonus_it));
  ...
  auto tg = draft_lm.topk_generate(
      *this, std::move(input_ids_with_bonus), std::move(accept_hidden_state_new),
...
// `this` is the draft.
LanguageModel::TopkGenerateResult
LanguageModel::topk_generate(LanguageModel& target_lm, std::vector<uint32_t> input_ids,
                             std::vector<Eigen::bfloat16> hidden_states, int num_cached_tokens,
                             bool is_prefill) {
  ...
  const int total_tokens =
      static_cast<int>(target_lm._cfg.lm_cfg.speculative_decoding_cfg.value().speculative_budget) -
      1;
  const int depth = 4;
  const int topk =
      static_cast<int>(_cfg.lm_cfg.speculative_decoding_cfg.value().speculative_budget);
  ...
  if (is_prefill) {
    ...
    for (size_t i = 0; i < offsets.size(); ++i) {
      ...
      std::vector<uint32_t> chunk_input_ids(input_ids.begin() + offset,
      ...
      draft_out = run_eagle3_draft_model(target_lm, std::move(chunk_hidden), chunk_input_ids,
    ...
    draft_out = run_eagle3_draft_model(target_lm, std::move(hidden_states), input_ids,
                                       /*attention_mask=*/std::nullopt,
  ...
  const uint32_t draft_vocab_size = _cfg.lm_cfg.get_lm_head_output_size();
  const Eigen::bfloat16* lm_output_draft = draft_out.logits.data() + last_row * draft_vocab_size;

  // Top-K over the lm_head logits row, then logsoftmax. Both shapes are
  // (1, topk) — leading 1 mirrors lm_output_draft's (1, vocab_size).
  auto topk_result = simaai::llima::topk<Eigen::bfloat16>(lm_output_draft, /*rows=*/1,
                                                          /*cols=*/draft_vocab_size, topk);
  ...
  for (size_t r = 0; r < topk_p_raw.size(); ++r) {
    topk_p[r] = logsoftmax<Eigen::bfloat16>(topk_p_raw[r].data(), topk_p_raw[r].size());
  }
  ...
  std::vector<Eigen::bfloat16> last_hidden(hidden_states.begin() + (hs_rows - 1) * hidden_size,
                                           hidden_states.end());

  // Broadcast last_hidden across all K rows for the depth loop.
  std::vector<Eigen::bfloat16> input_hidden(static_cast<size_t>(topk) * hidden_size);
  for (int i = 0; i < topk; ++i) {
    std::memcpy(input_hidden.data() + i * hidden_size, last_hidden.data(),
                hidden_size * sizeof(Eigen::bfloat16));
  }
  ...
  for (int i = 0; i < depth - 1; ++i) {
    ...
    auto draft_out_d =
        run_eagle3_draft_model(target_lm, std::move(input_hidden), input_ids,
                               /*attention_mask=*/std::nullopt,
    ...
    auto last_p_topk = simaai::llima::topk<Eigen::bfloat16>(draft_out_d.logits.data(),
    ...
    for (size_t r = 0; r < topk_p_raw_d.size(); ++r) {
      topk_p_d[r] = logsoftmax<Eigen::bfloat16>(topk_p_raw_d[r].data(), topk_p_raw_d[r].size());
    }
    ...
    auto cs_topk = simaai::llima::topk<float>(flat_cu.data(),
                                              /*rows=*/1,
                                              /*cols=*/static_cast<size_t>(topk) * topk, topk);
    ...
    std::vector<Eigen::bfloat16> new_input_hidden(static_cast<size_t>(topk) * embed_size_for_kv);
    for (size_t k = 0; k < topk_cs_index.size(); ++k) {
      const size_t src_row = static_cast<size_t>(out_ids[k]);
      std::memcpy(new_input_hidden.data() + k * embed_size_for_kv,
    ...
    // (tree_mask_init) along the col axis → (K, prev_cols + K).
    {
      ...
      std::vector<std::vector<std::vector<std::vector<float>>>> new_tree_data(
          1, std::vector<std::vector<std::vector<float>>>(
                 1, std::vector<std::vector<float>>(static_cast<size_t>(topk),
                                                    std::vector<float>(new_cols, 0.0f))));
      for (size_t r = 0; r < static_cast<size_t>(topk); ++r) {
        ...
        for (size_t c = 0; c < static_cast<size_t>(topk); ++c) {
          new_tree_data[0][0][r][prev_cols + c] = _eagle3_tree_mask_init.data[0][0][r][c];
        }
      }
  ...
  auto top_scores = simaai::llima::topk<float>(scores_list_flat.data(),
                                               /*rows=*/1,
                                               /*cols=*/scores_list_flat.size(), total_tokens);
  ...
  for (size_t i = 0; i < top_scores_index.size(); ++i) {
    draft_parents[i] = static_cast<int64_t>(draft_parents_flat[top_scores_index[i] / topk]);
  }
  ...
  for (size_t i = 0; i < draft_parents.size(); ++i) {
    if (draft_parents[i] == 0)
      mask_index[i] = -1;
  }
  for (size_t i = 0; i < mask_index.size(); ++i)
    mask_index[i] += 1;
  ...
  for (int i = 0; i < N; ++i)
    tree_mask_2d[i][i] = 1; // identity
  for (int i = 0; i < N; ++i)
    tree_mask_2d[i][0] = 1; // col 0 = 1
  ...
  std::vector<std::vector<int32_t>> retrieve_indices(leaf_num, std::vector<int32_t>(max_depth, -1));

  int rid = 0;
  for (int i = 0; i < total_tokens + 1; ++i) {
    if (noleaf_set.find(static_cast<int64_t>(i)) != noleaf_set.end())
      continue;
    int cid = i;
...
LanguageModel::InitTreeResult LanguageModel::initialize_tree(LanguageModel& draft_lm,
  ...
  if (prefix_cached == num_input_tokens && _cached_eagle3_prompt_len == num_input_tokens &&
  ...
  for (size_t i = 0; i < offsets.size(); ++i) {
    ...
    if (static_cast<size_t>(offset) + num_tokens <= static_cast<size_t>(prefix_cached)) {
      continue;
    }
    ...
    for (size_t cap_idx = 0; cap_idx < result.hidden_states.size(); ++cap_idx) {
      std::memcpy(result.hidden_states[cap_idx].data() + static_cast<size_t>(offset) * hidden_size,
  ...
  // Concat the 3 layer captures along the feature axis:
  // row i = [cap0[i], cap1[i], cap2[i]].
  std::vector<Eigen::bfloat16> hidden_states_concat(static_cast<size_t>(num_input_tokens) * 3 *
                                                    hidden_size);
  for (uint16_t i = 0; i < num_input_tokens; ++i) {
    for (size_t c = 0; c < 3; ++c) {
      std::memcpy(hidden_states_concat.data() + (i * 3 + c) * hidden_size,
  ...
  auto topk_result =
      draft_lm.topk_generate(*this, std::move(input_ids), std::move(hidden_states_concat),
...
std::optional<std::vector<uint32_t>> LanguageModel::run_model_speculative_decoding(
    ...
    std::optional<uint16_t> override_max_num_tokens, std::optional<ChronoTimer> timer_ttft) {
  ...
  const size_t spec_budget = _cfg.lm_cfg.speculative_decoding_cfg.value().speculative_budget;
  ...
  if (input_len + spec_budget > max_length) {
    _logger->info("stop (pre-init): input_len {} + spec_budget {} > max_length {}", input_len,
                  spec_budget, max_length);
  ...
  auto init = initialize_tree(draft_lm, input_ids, num_cached_tokens, timer_ttft.value());
  ...
  for (; idx < max_length; ++idx) {
    ...
    if (input_ids.size() + spec_budget > max_length) {
      _logger->info("[iter {}] stop (pre-verify): input_ids.size + spec_budget > max_length", idx);
      cache_full = true;
      break;
    }
    ...
    auto td = tree_decoding(draft_tokens, tree_position_ids, input_ids, retrieve_indices);
    ...
    std::vector<std::vector<int32_t>> candidates_2d(n_paths, std::vector<int32_t>(max_depth, -1));
    std::vector<int32_t> candidates_flat(n_paths * max_depth, -1);
    for (size_t p = 0; p < n_paths; ++p) {
      for (size_t d = 0; d < max_depth; ++d) {
        const int32_t r_idx = retrieve_indices[p][d];
        if (r_idx >= 0) {
          const int32_t tok = static_cast<int32_t>(draft_tokens[r_idx]);
    ...
    auto post = eagle_helpers::evaluate_posterior(td.logits.data(), candidates_flat.data(), n_paths,
                                                  max_depth, td.vocab_size);
    ...
    if (final_batch) {
      _logger->info("[iter {}] commit accepted path and stop: no room for next tree", idx);
      cache_full = true;
    }
    ...
    if (final_batch) {
      for (size_t i = 0; i < accepted_count; ++i) {
        input_ids.push_back(static_cast<uint32_t>(candidates_2d[post.best_candidate][i]));
      }
    } else {
      auto upd = update_inference_inputs(draft_lm, input_ids, candidates_2d, post.best_candidate,
                                         post.accept_length, retrieve_indices, new_token,
                                         td.hidden_states, post.sample_p);
    ...
    const double iter_duration = std::chrono::duration<double>(iter_end - iter_begin).count();
    const size_t stream_begin = prev_size + (first_round ? 1 : 0);
    const size_t streamed = input_ids.size() - stream_begin;
    const double per_token = streamed > 0 ? iter_duration / static_cast<double>(streamed) : 0.0;
    ...
    if (std::any_of(input_ids.begin() + input_len, input_ids.end(),
  ...
  // TPS summary.
  const auto run_model_duration =
      std::chrono::duration<double>(std::chrono::steady_clock::now() - run_model_begin).count();
  const size_t rounds = idx + 1;
  const size_t total_tokens =
      std::accumulate(total_generated_tokens.begin(), total_generated_tokens.end(), size_t{0});
  _logger->info(
      "EAGLE generated {} tokens in {:.2f}s. TPS = {:.2f}", total_tokens, run_model_duration,
      run_model_duration > 0.0 ? static_cast<double>(total_tokens) / run_model_duration : 0.0);
  _logger->info(
      "DECODING completed in {} rounds ({:.2f} tokens generated per round) | {:.2f}s per round",
      rounds, static_cast<double>(total_tokens) / static_cast<double>(rounds),
  ...
  _text_streamer.push(cache_full ? DecodeCallbackType::CACHE_FULL : DecodeCallbackType::STOP, 0, 0);
  _text_streamer.wait_streaming();
