...
public:
  ...
  virtual std::vector<uint32_t> encode(const std::string& text, bool add_special_tokens) const = 0;
  virtual std::string decode(const std::vector<uint32_t>& ids, bool skip_special_tokens) const = 0;
  virtual std::string id_to_token(uint32_t id) const = 0;
  virtual uint32_t token_to_id(const std::string& token) const = 0;
  virtual std::string get_chat_template() const {
    return "";
  }
  virtual std::string get_bos_token() const {
    return "";
  }
  virtual uint32_t get_bos_token_id() const {
    return std::numeric_limits<uint32_t>::max();
  }
  virtual std::string get_eos_token() const {
    return "";
  }
  ...
  from_hf_json(const std::filesystem::path tokenizer_json_file_name);
...
public:
  ...
  virtual std::vector<uint32_t> encode(const std::string& text,
                                       bool add_special_tokens) const override {
    TokenizerEncodeResult result;
    tokenizers_encode(_handle, text.data(), text.length(), static_cast<int>(add_special_tokens),
                      &result);
  ...
  virtual std::string decode(const std::vector<uint32_t>& ids,
                             bool skip_special_tokens) const override {
    tokenizers_decode(_handle, ids.data(), ids.size(), static_cast<int>(skip_special_tokens));
    const char* data;
    size_t len;
...
public:
  ...
  virtual std::vector<uint32_t> encode(const std::string& text,
    ...
    const auto num_tokens =
        -llama_tokenize(_vocab, text.c_str(), text.size(), nullptr, 0, add_special_tokens, true);
    std::vector<uint32_t> tokens(num_tokens);
    const auto rc = llama_tokenize(_vocab, text.c_str(), text.size(),
                                   reinterpret_cast<llama_token*>(tokens.data()), tokens.size(),
