...
inline std::vector<Eigen::bfloat16> make_causal_mask(size_t tgt_len, size_t past_kv_len) {
  ...
  for (size_t i = 0; i < tgt_len; ++i) {
    ...
    for (size_t j = 0; j < allowed_end; ++j)
      row[j] = zero;
    for (size_t j = allowed_end; j < total_cols; ++j)
      row[j] = neg_inf;
  }
  return mask;
}
...
inline void apply_tree_mask(std::vector<Eigen::bfloat16>& mask, size_t mask_rows, size_t mask_cols,
...
inline std::vector<Eigen::bfloat16>
prepare_attention_mask(const std::optional<std::vector<uint8_t>>& attention_mask, size_t seq_length,
  ...
  if (tree_mask && !tree_mask->data.empty() && !tree_mask->data[0].empty() &&
...
inline EvaluatePosteriorResult evaluate_posterior(const Eigen::bfloat16* logits,
                                                  const int32_t* candidates, size_t num_paths,
                                                  size_t candidate_len, size_t vocab_size) {
...
#pragma omp parallel for schedule(static)
  ...
  const Eigen::bfloat16* sample_row = logits + (best_c * candidate_len + max_accept) * vocab_size;
...
// std::upper_bound.
template <typename T>
std::vector<size_t> searchsorted(const std::vector<T>& sorted_arr, const std::vector<T>& queries,
                                 bool right = false) {
  std::vector<size_t> result(queries.size());
  for (size_t i = 0; i < queries.size(); ++i) {
    auto it = right ? std::upper_bound(sorted_arr.begin(), sorted_arr.end(), queries[i])
