...
public:
  ...
  const nlohmann::ordered_json& get_messages() const {
    return _messages;
  }
  void clear_messages();

  void set_tools(nlohmann::ordered_json tools);
  const nlohmann::ordered_json& get_tools() const {
    return _tools;
  }
  bool has_tools() const {
    return _tools.is_array() && !_tools.empty();
  }

  void add_image(std::filesystem::path image_path);
  void clear_images();
  const std::vector<std::filesystem::path>& get_images() const {
    return _images;
  }
  ...
  void print_history() const {
    std::cout << _messages << std::endl << std::flush;
  }
