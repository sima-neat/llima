...
VisionLanguageModel::VisionLanguageModel(std::filesystem::path model_path,
                                         std::optional<std::string> system_prompt,
                                         std::optional<std::string> chat_template)
    : BaseModel(model_path), _vlm_helper(_cfg, _devkit_dir, system_prompt, chat_template),
  ...
  _language_model_ptr = std::make_unique<LanguageModel>(
      ...
      _vlm_helper.get_pad_token_id(), _text_streamer);
...
std::optional<std::string> VisionLanguageModel::run_model(const Chat& chat,
  ...
  // vision models.
  auto vision_ofm_maps =
      _language_model_ptr->create_input_buffers(preprocessed_data.input_token_ids);
  ...
  if (_draft_vlm_ptr != nullptr) {
    output_token_ids = _language_model_ptr->run_model_speculative_decoding(
        *_draft_vlm_ptr->_language_model_ptr, preprocessed_data.input_token_ids, max_num_tokens,
        timer_ttft);
  } else {
    output_token_ids = _language_model_ptr->run_model(preprocessed_data.input_token_ids, timer_ttft,
                                                      max_num_tokens);
  }
...
std::vector<uint32_t>
VisionLanguageModel::run_model(std::span<const uint32_t> input_token_ids,
...
VisionLanguageModel::run_model_for_logits(std::span<const uint32_t> input_token_ids) {
...
std::vector<double>
VisionLanguageModel::run_model_for_ttnt(std::span<const uint32_t> input_token_ids,
                                        std::optional<uint16_t> override_max_num_tokens,
                                        std::optional<std::set<uint32_t>> override_stop_token_ids) {
  ...
  if (!_language_model_ptr->get_stop_token_ids().contains(next_token_id)) {
    ...
    auto original_max_num_tokens = _language_model_ptr->set_max_num_tokens(override_max_num_tokens);
    auto original_stop_token_ids = _language_model_ptr->set_stop_token_ids(override_stop_token_ids);

    // Generate the new tokens until the stop token.
    for (size_t i = input_token_ids.size(); i < _language_model_ptr->get_max_num_tokens(); ++i) {
      next_token_id =
          _language_model_ptr->run_model_once(1, i, input_token_ids.size(), next_token_id);
      ttnt.emplace_back(timer.stop(true));
