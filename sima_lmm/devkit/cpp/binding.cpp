...
NB_MODULE(cpp_ext, m) {
  ...
  m.def(
      "connect_cpp",
      [](const std::vector<std::string>& mla_rt_args, const std::string& log_file_name,
         int log_level) {
        ...
        case 50:
          ...
          break;
        default:
          std::runtime_error("Unable to convert python logging level to spdlog logging level: " +
                             std::to_string(log_level));
        }
        connect(mla_rt_args, log_file_name, spdlog_log_level);
      },
      nb::arg("mla_rt_args"), nb::arg("log_file_name"), nb::arg("log_level") = 0);
  m.def("disconnect_cpp", &disconnect);

  nb::class_<CLI>(m, "CLI")
      .def(nb::init<std::filesystem::path, std::optional<std::filesystem::path>,
                    std::optional<std::filesystem::path>, std::optional<std::string>,
                    std::optional<std::string>>(),
           nb::arg("model_path"), nb::arg("whisper_model_path") = nb::none(),
           nb::arg("draft_model_path") = nb::none(), nb::arg("system_prompt") = nb::none(),
