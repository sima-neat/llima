# Copied from https://github.com/openai/whisper/blob/main/whisper/tokenizer.py with some
# modifications.
...
class Tokenizer:
    ...
    def decode_without_special_tokens(self, token_ids: list[int], **kwargs) -> str:
...
# Copied from HuggingFace transformers.models.gpt2.tokenization_gpt2.bytes_to_unicode with some
# modification.
...
def get_encoding(
    name: str = "gpt2", num_languages: int = 99, hf_tokenizer_json_file: Path | None = None
):
    try:
        if hf_tokenizer_json_file is None:
            # Find the tiktoken file if HuggingFace tokenizer.json file is not found.
            vocab_path = Path(__file__).parent / f"{name}.tiktoken"
            ranks = {
                base64.b64decode(token): int(rank)
                for token, rank in (line.split() for line in open(vocab_path) if line)
            }
        else:
            # Construct the ranks from tokenizer.json file.
            with open(hf_tokenizer_json_file, "r") as f:
            ...
            for text, token_id in vocab.items():
                if text == "<|endoftext|>":
                    continue
                ranks[bytes([byte_decoder[c] for c in text])] = token_id
    except Exception as e:
        if hf_tokenizer_json_file is None:
            msg = f"Unable to load the vocab file: {vocab_path}"
        else:
            msg = f"Unable to load the tokenizer file: {hf_tokenizer_json_file}"
...
def get_tokenizer(
    ...
    *,
    ...
    hf_tokenizer_json_file: Path | None = None,
) -> Tokenizer:
    ...
    encoding = get_encoding(
        name=encoding_name, num_languages=num_languages,
