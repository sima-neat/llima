...
public:
  Qwen3TtsRunner(std::filesystem::path model_dir,
                 std::filesystem::path components_dir, bool preload_models);
  ~Qwen3TtsRunner();
  void initialize();
  void finalize();
...
private:
  ...
  NativeTensor text_project(const NativeTensor &input) const;
  NativeTensor build_prefill(const std::vector<uint32_t> &ids,
                             const std::string &speaker,
                             const std::string &language,
                             NativeTensor &trailing, NativeTensor &pad) const;
  NativeTensor run_backbone_prefill(const NativeTensor &prefill,
                                    const std::string &speaker,
                                    const std::string &language,
                                    RunMetrics &metrics);
  void run_backbone_token(uint16_t token);
  std::array<int32_t, 16> run_code_predictor(const NativeTensor &hidden,
                                             int32_t c0,
                                             const RequestOptions &request,
                                             RunMetrics &metrics);
  NativeTensor
  run_codec_prefix(const std::vector<std::array<int32_t, 16>> &codes);
  float
  codec_prefix_tail_rms(const std::vector<std::array<int32_t, 16>> &frames);
  NativeTensor run_codec_transformer(const NativeTensor &hidden);
  NativeTensor run_codec_transformer_n128_hybrid(const NativeTensor &hidden);
  std::vector<float> run_codec_tail(const NativeTensor &hidden);
  int32_t select_token(const NativeTensor &logits,
                       const std::vector<int32_t> &previous, bool suppress,
                       bool sample, uint32_t top_k, float top_p,
                       float temperature, float repetition_penalty);
  NativeTensor codec_embedding(int32_t token, uint32_t codebook) const;
  NativeTensor backbone_feedback(const std::array<int32_t, 16> &frame) const;
  NativeTensor bf16_download(const std::string &name) const;
  void bf16_upload(const std::string &name, const NativeTensor &value) const;
  ...
  void fp32_upload(const std::string &name, const NativeTensor &value) const;
#endif
  ...
  std::map<ModelKey, ModelPtr> codec_n128_pre_, codec_n128_cache_,
      codec_n128_post_;
  std::map<ModelKey, ModelPtr> codec_final_pre_from_n128_,
      codec_final_post_from_n128_;
  ...
  NumpyPcg64 rng_;
  std::unique_ptr<TensorFile> backbone_weights_, cp_weights_, codec_weights_,
      text_projection_weights_, codec_head_weights_;
  const NativeTensorView *text_embeddings_{}, *codec_head_weight_{},
      *text_fc1_w_{}, *text_fc1_b_{}, *text_fc2_w_{}, *text_fc2_b_{};
  std::array<const NativeTensorView *, 16> codec_embeddings_{};
