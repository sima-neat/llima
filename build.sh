...
ensure_neat_internals() {
  ...
  # Install every package the selected Internals artifact delivered.  Internals
  # owns which packages that is; LLiMa does not keep a list of its own.
  local debs=()
  local deb
...
write_install_manifest() {
  ...
  # Every Internals package precedes the LLiMa packages.  Which packages the
  # Internals artifact contains is Internals' decision, so nothing here names
  # them.
  while IFS= read -r file; do
    basename_file="$(basename "${file}")"
