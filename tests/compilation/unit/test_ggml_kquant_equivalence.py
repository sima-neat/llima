...
@pytest.mark.parametrize(
    ("unpacker", "widths"),
    [
        (ggml_quant.unpack_q6_k_quants, (128, 64)),
        (ggml_quant.unpack_q5_k_quants, (128, 32)),
        (ggml_quant.unpack_q4_k_quants, (128,)),
        (ggml_quant.unpack_q3_k_quants, (64, 32)),
        (ggml_quant.unpack_q2_k_quants, (64,)),
    ],
    ids=lambda value: getattr(value, "__name__", None),
)
def test_value_unpackers_peak_memory_stays_near_output_size(unpacker, widths):
    """
    Guard the chunked, in-place decoding: peak memory must stay close to the size of the
    decoded output, however large the tensor.  Model tensors can hold a billion values, so
    a full-size temporary would cost a gigabyte per copy.
    """
    import tracemalloc

    n_super = 16 * ggml_quant.UNPACK_CHUNK_SUPERBLOCKS   # 16 chunks, 32 MB of output
    rng = np.random.default_rng(7)
    fields = [rng.integers(0, 256, size=(n_super, w), dtype=np.uint8) for w in widths]
    output_bytes = n_super * 256
    ...
    assert peak < 1.25 * output_bytes, f"peak {peak / output_bytes:.2f}x of the output size"
