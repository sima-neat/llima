...
int main() {
  ...
  try {
    const std::filesystem::path model_dir =
        simaai::llima::test::resolve_model_dir(kModelEnv, simaai::llima::test::kDefaultVlmModelName,
                                               "LLiMa VLM", "devkit/vlm_config.json");
    const std::filesystem::path image = simaai::llima::test::resolve_asset("sjc.jpg");
    std::cout << "LLIMA_VLM model_dir=" << model_dir << '\n';
    std::cout << "LLIMA_VLM image=" << image << '\n';

    simaai::llima::connect({}, "/tmp/sima_lmm_vision_generation_test.log", spdlog::level::info);
    connected = true;
