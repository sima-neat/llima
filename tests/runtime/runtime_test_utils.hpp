...
inline std::filesystem::path resolve_model_dir(const char* model_env,
  ...
  if (model_name.empty() || model_name.front() == '/' ||
  ...
  if (!std::filesystem::is_regular_file(model_dir / expected_config)) {
    throw std::runtime_error(std::string(model_env) + " resolves to a missing or invalid " +
