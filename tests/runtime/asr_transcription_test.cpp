...
int main() {
  ...
  try {
    const std::filesystem::path model_dir =
        simaai::llima::test::resolve_model_dir(kModelEnv, simaai::llima::test::kDefaultAsrModelName,
    ...
    simaai::llima::connect({}, "/tmp/sima_lmm_asr_transcription_test.log", spdlog::level::info);
    connected = true;
    ...
    if (!std::isfinite(result.no_speech_prob) || result.no_speech_prob < 0.0F ||
