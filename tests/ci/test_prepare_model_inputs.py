...
def test_resolve_model_plan_accepts_atypical_safetensors_shard() -> None:
    source = prepare_model_inputs.SourceSpec(
        repo_id="org/model",
        revision="main",
        payload_format="safetensors",
        cache_root="llima-safetensors",
    )
    shard = "model.safetensors-00001-of-00001.safetensors"
    patterns = (*prepare_model_inputs.SELECTION_PATTERNS["safetensors"], shard)
    prefix = "llima-safetensors/org/model/latest"
    manifest = {
        "repo_id": source.repo_id,
        "requested_revision": source.revision,
        "resolved_revision": "a" * 40,
        "format": source.payload_format,
        "selection_sha256": prepare_model_inputs.selection_fingerprint(
            source.payload_format, patterns
        ),
        "files": [
            {
                "path": shard,
                "s3_key": f"{prefix}/{shard}",
                "size": 1,
                "sha256": "b" * 64,
            }
        ],
    }
    artifact_source = prepare_model_inputs.ArtifactSource(
        base_url="https://artifacts.neat.sima.ai"
    )

    with mock.patch.object(
        prepare_model_inputs, "fetch_manifest", return_value=manifest
    ):
        plan = prepare_model_inputs.resolve_model_plan(artifact_source, source)

    assert [cached.relative_path.as_posix() for cached in plan.files] == [shard]
