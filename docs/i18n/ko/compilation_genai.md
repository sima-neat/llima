...
| `--future_token_mask_size` | 토큰 위치 간에 컴파일된 모델을 재사용하기 위한 마스크 크기입니다. 값이 클수록 컴파일된 바이너리 파일의 수가 줄어들지만, 초당 처리량(TPS)이 감소할 수 있습니다. 기본값: 128. |
| `--enable_filter_sharing` | 그룹 모델과 단일 모델 간에 필터 공유를 활성화하여 DRAM 사용량을 줄이되, TTFT(총 처리 시간)는 증가하고 TPS(초당 처리량)는 감소합니다. 이는 두 모델 유형 모두 동일한 정밀도를 사용할 때만 효과가 있으며, LoRA를 사용하여 컴파일할 때 필수적으로 적용해야 합니다. |
