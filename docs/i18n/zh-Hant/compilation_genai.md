...
| 進階論證 | 描述 |
|----|----|
| `--language_group_size` | 在預填階段，用於平行處理分詞的批次大小。較大的值（例如，256）可以提高大型輸入提示的「首次產生文字所需時間」（TTFT），但可能會降低小型輸入提示的 TTFT。預設值：128。 |
| `--future_token_mask_size` | 用於跨分詞位置重複使用已編譯模型的遮罩大小。較大的值會減少已編譯的二進位檔案數量，但可能會降低每秒交易次數 (TPS)。預設值：128。 |
| `--enable_filter_sharing` | 啟用群組模型和單一模型之間的篩選器共享功能，以減少 DRAM 使用量，但會增加 TTFT（總推論時間）並降低 TPS（每秒交易次數）。此功能僅在兩種模型類型都使用相同的精確度時才有效，並且在使用 LoRA 進行編譯時是必需的。 |
