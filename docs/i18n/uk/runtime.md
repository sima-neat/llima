...
| `clear image` | Видалити зображення, поставлені в чергу після останнього надісланого запиту; зображення з попередніх кроків залишаються в історії. |
| `set system <prompt>` | Встановіть системне повідомлення. |
| `clear system` | Очистіть системне повідомлення, історію чату та зображення. |
| `clear history` | Видалити надіслані запити, відповіді та всі зображення, зберігши системний запит. |
